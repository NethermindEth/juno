...
func TestKey(t *testing.T) {
	t.Run("bucket with no key", func(t *testing.T) {
		key := db.DeprecatedStateTrie.Key()
		assert.Equal(t, []byte{byte(db.DeprecatedStateTrie)}, key)
	})
	t.Run("bucket with nil key", func(t *testing.T) {
		key := db.DeprecatedStateTrie.Key(nil)
		assert.Equal(t, []byte{byte(db.DeprecatedStateTrie)}, key)
	})
	t.Run("bucket with multiple keys", func(t *testing.T) {
		...
		for _, k := range keys {
			t.Run(string(rune(len(k))), func(t *testing.T) {
				...
				expectedKey = append(expectedKey, byte(db.DeprecatedStateTrie))
				expectedKey = append(expectedKey, k...)
				assert.Equal(t, expectedKey, db.DeprecatedStateTrie.Key(k))
			})
		}
	})
}
