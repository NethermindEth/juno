...
func TestRemote(t *testing.T) {
	...
	remoteDB, err := New(l.Addr().String(), context.Background(), utils.NewNopZapLogger(), grpc.WithTransportCredentials(insecure.NewCredentials()))
	require.NoError(t, err)

	t.Run("Get", func(t *testing.T) {
		assert.NoError(t, remoteDB.View(func(txn db.Snapshot) error {
			for i := byte(0); i < 3; i++ {
				val, err := txn.Get([]byte{i})
				if err != nil {
					return err
				}
		...
		val, err := remoteDB.Get([]byte{0xDE, 0xAD})
