...
func TestManager_PutReceipt(t *testing.T) {
	...
	for _, r := range receipts {
		if err := TransactionService.StoreReceipt(r.ReceiptHash, r.Receipt); err != nil {
			t.Error(err)
		}
	}
	TransactionService.Close(context.Background())
...
func TestManager_GetReceipt(t *testing.T) {
	...
	for _, r := range receipts {
		outReceipt, err := TransactionService.GetReceipt(r.ReceiptHash)
		if err != nil {
			if errors.Is(err, db.ErrNotFound) {
				t.Errorf("receipt not found: %s", r.ReceiptHash.String())
			}
			t.Error(err)
		}
