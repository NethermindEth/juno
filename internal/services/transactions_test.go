...
var (
	invokeTransactions = []transaction.InvokeFunctionTransaction{
		{
			transaction.Common{
				TxHash: fromHexString("49eb3544a95587518b0d2a32b9e456cb05b32e0085ebc0bcecb8ef2e15dc3a2"),
				TxType: transaction.TxInvokeFunction,
			},
			transaction.InvokeFunctionFields{
	...
	deployTransactions = []transaction.DeployTransaction{
		{
			transaction.Common{
				TxHash: fromHexString("e0a2e45a80bb827967e096bcf58874f6c01c191e0a0530624cba66a508ae75"),
				TxType: transaction.TxDeploy,
			},
			transaction.DeployFields{
...
func TestTransactionService_StoreTransaction(t *testing.T) {
	...
	for _, tx := range invokeTransactions {
		TransactionService.StoreTransaction(tx.AsTransaction())
	}
...
func compareCommon(a, b transaction.Common) bool {
	return a.TxType == b.TxType &&
		a.TxHash.Cmp(&b.TxHash) == 0
}

func compareDeployFields(a, b transaction.DeployFields) bool {
	return a.ContractAddressSalt.Cmp(&b.ContractAddressSalt) == 0 &&
		compareBigIntArray(a.ConstructorCalldata, b.ConstructorCalldata)
}

func compareInvokeFunctionFields(a, b transaction.InvokeFunctionFields) bool {
...
func compareTransaction(a, b *transaction.Transaction) bool {
	if !compareCommon(a.Common, b.Common) {
		return false
	}
	if a.TxType == transaction.TxDeploy {
		aD := a.AsDeploy()
		bD := a.AsDeploy()
		return compareDeployFields(aD.DeployFields, bD.DeployFields)
	}
	if a.TxType == transaction.TxInvokeFunction {
