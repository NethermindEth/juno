...
func New(stateManager *statedb.Manager, logger log.Logger) *VirtualMachine {
...
func (s *VirtualMachine) Run(dataDir string) error {
	...
	if err := s.vmCmd.Start(); err != nil {
		s.logger.Errorw("Failed to start python vm rpc", "error", err)
		return err
	}
	...
	// Run the gRPC server.
	go func() {
		s.logger.Infow("gRPC server listening", "address", lis.Addr())
		if err := s.rpcServer.Serve(lis); err != nil {
			// notest
			s.logger.Errorw("Failed to serve", "error", err)
		}
	}()
...
func (s *VirtualMachine) Call(
	...
	if err != nil {
		s.logger.Errorw("failed to dial", "error", err)
		return nil, err
	}
	defer conn.Close()
	...
	if err != nil {
		s.logger.Errorw("Failed to call", "error", err)
		return nil, err
	}
