...
- [Hardware Requirements](hardware-requirements): Size a machine for a validator, a dApp backend, or an RPC provider

- [Running a Juno Node](running-juno): Start a node with Docker, a prebuilt binary, or from source

- [Syncing from a Snapshot](snapshots): Start from a recent block instead of replaying the chain from genesis

- [Configuring Juno](configuring): Set options with flags, environment variables, or a YAML file

- [Interacting with Juno](json-rpc): Call the node over JSON-RPC and subscribe to updates over WebSocket
