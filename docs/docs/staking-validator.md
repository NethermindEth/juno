...
Juno is a reliable choice for running a Starknet node. Follow the [Running Juno](running-juno) guide to configure Juno using Docker, binaries, source builds, Google Cloud Platform (GCP), or [Kubernetes with Helm](running-on-kubernetes).
