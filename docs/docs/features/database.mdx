...
**Juno** defines a database as the union of the above operations and a **Close** function to friendly close the database,
...
**RunTxn** to run a batch of operations in the database inside of a unique transaction. This behavior is described by
...
Now **Juno** uses an MDB database to store all its data. An MDB database is an environment (a database file) that can
have multiple scopes or named databases. With that characteristic, **Juno** has only one MDB environment and multiple
named databases, one for each database on the application.

To know more about the MDB Database and the package **Juno** uses, see these links:
