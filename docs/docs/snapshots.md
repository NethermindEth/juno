...
It is possible avoid syncing from the beginning and waiting weeks to catch-up by downloading a Juno snapshot. You're downloading a pre-synced Juno database that you can point your node to. This will reduce the syncing time to just a few hours.
...
Additionally, _pruned_ snapshots are offered, they contain only the latest data, diminishing by a lot storage size.
