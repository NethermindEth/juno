...
In this short article we lay out the steps to get a Raspberry PI with a working [Raspbian](https://www.raspbian.org/) installation to run the Juno StarkNet node. We assume that you have access as a root user to the Pi.
...
For the latest on the project, check out our [Github repo](https://github.com/NethermindEth/juno), and refer to our [Documentation](https://gojuno.xyz/).

## Main Features 
<!-- TODO: Any other relevant features to tell here? (REST API, Metrics(?), Docker APP version) -->
...
### Specification 🤖
    ...
    - USB keyboard, a monitor, and an HDMI cable to interact with the computer.
...
Now extract the tarball into your /usr/local/ directory.
...
If you have no preferred text editor, you can use `nano ~/.bashrc` to edit your bash config.
...
Finally, as your `.bashrc` is only initialised when the shell is started, you need to source the new `~/.bashrc`.

```bash
source ~/.bashrc
```

You should now have a working Golang installation in your Raspberry Pi.
...
## Configuring juno
<!-- TODO: Marcos please check below -->
Usually, when you first run Juno, a [config file](https://gojuno.xyz/docs/running/config) will be created in `XXYYZZ.juno.yaml` in your home directory. 

When syncing the StarkNet state from a Raspberry Pi, we expect you to have an SSD drive of at least XXXGB. You should make sure that you set the database location within it.

You shoule ensure that the property `db_path` points to the external SSD:
