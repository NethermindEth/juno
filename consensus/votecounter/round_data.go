...
func (r *roundData[V, H, A]) addVote(vote *types.Vote[H, A], votingPower types.VotingPower, voteType VoteType) bool {
	...
	perVote.add(&vote.Sender, votingPower, voteType)
...
func (r *roundData[V, H, A]) getQuorumForVote(voteType VoteType, id *H) types.VotingPower {
	var perVote *ballotSet[A]
	if id != nil {
...
func (r *roundData[V, H, A]) getQuorumForAny(voteType VoteType) types.VotingPower {
	return r.allVotes.perVoteType[voteType]
}

func (r *roundData[V, H, A]) getNonFaultyFutureMessage() types.VotingPower {
