...
func NewProtoBroadcaster(log utils.Logger, bufferSize int) ProtoBroadcaster {
...
func (b ProtoBroadcaster) Loop(ctx context.Context, topic *pubsub.Topic) {
	for {
		...
		case msg := <-b.ch:
			...
			for {
				if err := topic.Publish(ctx, msgBytes, pubsub.WithReadiness(pubsub.MinTopicSize(1))); err != nil && !errors.Is(err, context.Canceled) {
