...
func TestTransition(t *testing.T) {
	...
	transition := NewTransition[value, felt.Felt, felt.Felt](val)
	...
	t.Run("Valid Empty Block", func(t *testing.T) {
		...
		commitState := &ReceivingTransactionsState{
			Header:     header,
			ValidRound: -1,
			Value:      nil,
		}
		proposalCommitMsg := &consensus.ProposalCommitment{
			...
			ConcatenatedCounts:        &common.Felt252{Elements: []byte{0}},
		...
		// 3. OnProposalFin
		finState := &AwaitingProposalFinState{
			Header:     header,
			ValidRound: -1,
			Value:      nil,
		}
	...
	t.Run("Valid NonEmpty Block", func(t *testing.T) {
		...
		waitingState := &AwaitingBlockInfoOrCommitmentState{
			Header:     header,
			ValidRound: -1,
		}
		...
		txnState := &ReceivingTransactionsState{
			Header:     header,
			ValidRound: -1,
			Value:      nil,
		}
