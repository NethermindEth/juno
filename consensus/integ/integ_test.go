...
type testConfig struct {
	...
	targetHeight       tendermint.Height
}

func getTimeoutFn(cfg testConfig) func(tendermint.Step, tendermint.Round) time.Duration {
	return func(step tendermint.Step, round tendermint.Round) time.Duration {
...
func runTest(t *testing.T, cfg testConfig) {
	...
	// node index -> height
	heights := make([]tendermint.Height, honestNodeCount)
