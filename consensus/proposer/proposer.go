...
func (p *proposer) ProposalInit() (types.ProposalInit, error) {
	...
	if err := p.builder.InitPendingBlock(); err != nil {
		return types.ProposalInit{}, err
	}
...
func (p *proposer) Txns(ctx context.Context) <-chan TxnBatchResult { // Todo: consider back pressure
	...
	go func() {
		...
		for {
			...
			txns, err := p.builder.PopBatchTxns()
			if err != nil {
			...
			case <-ctx.Done():
				return
			case out <- TxnBatchResult{Txns: adaptedTxns}:
			}
		}
	}()
...
// ProposalFin() commits the pending block, and returns its block hash
...
func (p *proposer) ProposalFin() (types.ProposalFin, error) {
	pblock := p.builder.PendingBlock()
	if err := p.builder.Finalise(nil); err != nil {
		return types.ProposalFin{}, err
	}
