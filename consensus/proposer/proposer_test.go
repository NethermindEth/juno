...
func TestProposer(t *testing.T) {
	...
	t.Run("Initial state", func(t *testing.T) {
		...
		assert.Empty(t, p.Pending().Block.Transactions)
	})
	...
	t.Run("Receive transactions", func(t *testing.T) {
		for i, batch := range firstBatches {
			t.Run(fmt.Sprintf("Batch size %d", len(batch)), func(t *testing.T) {
				submit(p, batch)
				assert.Equal(t, slices.Concat(firstBatches[:i+1]...), p.Pending().Block.Transactions)
			})
		}
	})
	...
	t.Run("Another node build the first 2 batches", func(t *testing.T) {
		...
		for i, batch := range committedFirstBatches {
			t.Run(fmt.Sprintf("Batch size %d", len(batch)), func(t *testing.T) {
				...
				assert.Equal(t, slices.Concat(committedFirstBatches[:i+1]...), otherProposer.Pending().Block.Transactions)
			})
		}
	...
	t.Run("Commit", func(t *testing.T) {
		...
		t.Run(fmt.Sprintf("Should process the pending %d transactions", count(committedSecondBatches)), func(t *testing.T) {
			time.Sleep(time.Duration(count(committedSecondBatches)) * waitPerTransaction)
			assert.NotNil(t, p.Pending())
			assert.Equal(t, slices.Concat(committedSecondBatches...), p.Pending().Block.Transactions)
		})
...
func submit(proposer proposer.Proposer[starknet.Value, starknet.Hash], batch []core.Transaction) {
	...
	time.Sleep(time.Duration(len(batch)) * waitPerTransaction)
}
