...
func initNode(
...
) driver.Driver[starknet.Value, starknet.Hash, starknet.Address] {
	defer func() {
	...
	stateMachine := tendermint.New(tendermintDB, logger, nodeAddr, proposer, validators, types.Height(0))
	...
	return driver.New(logger, tendermintDB, stateMachine, bc, p2p, proposer, getTimeoutFn(cfg))
}
...
func runTest(t *testing.T, cfg testConfig) {
	...
	mapper := iter.Mapper[networkNodeConfig, driver.Driver[starknet.Value, starknet.Hash, starknet.Address]]{
		MaxGoroutines: honestNodeCount,
	}
	drivers := mapper.Map(p2pHosts, func(p2pHost *networkNodeConfig) driver.Driver[starknet.Value, starknet.Hash, starknet.Address] {
		return initNode(t, &validators, allNodes.addr[p2pHost.index], p2pHost, logger, commits, &cfg, genesisDiff, genesisClasses)
	})
	...
	for i := range drivers {
		wg.Go(func() {
			require.NoError(t, drivers[i].Run(ctx))
		})
	}
	t.Cleanup(func() {
		cancel()
