...
func TestSkipRound(t *testing.T) {
	t.Run(
		"Line 55 (Proposal): Start round r' when f+1 future round messages are received from round r'",
		func(t *testing.T) {
	...
	t.Run(
		"Line 55 (Prevote): Start round r' when f+1 future round messages are received from round r'",
		func(t *testing.T) {
			stateMachine := setupStateMachine(t, 4, 3)
			expectedHeight := types.Height(0)
			rPrime, rPrimeVal := types.Round(4), value(10)
	...
	t.Run(
		"Line 55 (Precommit): Start round r' when f+1 future round messages are received from round r'",
		func(t *testing.T) {
			stateMachine := setupStateMachine(t, 4, 3)
			expectedHeight := types.Height(0)
