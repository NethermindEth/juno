...
func (cs *Tendermint[V, H, A]) readReplayMessage(msg *TimedWALMessage, newStepSub types.Subscription) error {
	// Todo (rian)
	...
	// Todo (rian): need to define the message types (votes, timeouts, block commits, etc)
	switch m := msg.Msg.(type) {
	case types.EventDataRoundState:
		cs.log.Infow("Replay: New Step", "height", m.Height, "round", m.Round, "step", m.Step)
	...
	case msgInfo:
		...
		case *ProposalMessage:
			p := msg.Proposal
			cs.log.Infow("Replay: Proposal", "height", p.Height, "round", p.Round, "header",
				p.BlockID.PartSetHeader, "pol", p.POLRound, "peer", peerID)
		case *BlockPartMessage:
			cs.log.Infow("Replay: BlockPart", "height", msg.Height, "round", msg.Round, "peer", peerID)
		case *VoteMessage:
			v := msg.Vote
			cs.log.Infow("Replay: Vote", "height", v.Height, "round", v.Round, "type", v.Type,
				"blockID", v.BlockID, "peer", peerID)
		}

		cs.handleMsg(m)
	case timeoutInfo:
		cs.log.Infow("Replay: Timeout", "height", m.Height, "round", m.Round, "step", m.Step, "dur", m.Duration)
...
func (cs *Tendermint[V, H, A]) catchupReplay(csHeight int64) error {
