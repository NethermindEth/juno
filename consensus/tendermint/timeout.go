...
func (s *stateMachine[V, H, A]) onTimeoutPropose(timeout types.Timeout) []actions.Action[V, H, A] {
	...
	if isSameHeightAndRound && isProposeStep {
		return []actions.Action[V, H, A]{
			&actions.WriteWAL[V, H, A]{Entry: (*wal.WALTimeout)(&timeout)},
...
func (s *stateMachine[V, H, A]) onTimeoutPrevote(timeout types.Timeout) []actions.Action[V, H, A] {
	isSameHeightAndRound := s.state.height == timeout.Height && s.state.round == timeout.Round
...
func (s *stateMachine[V, H, A]) onTimeoutPrecommit(timeout types.Timeout) []actions.Action[V, H, A] {
	isSameHeightAndRound := s.state.height == timeout.Height && s.state.round == timeout.Round
