...
func TestFirstProposal(t *testing.T) {
	t.Run(
		"Line 22: valid proposal with lockedRound = -1 should prevote for proposal",
		func(t *testing.T) {
			...
			currentRound.validator(0).
				proposal(proposalValue, -1).
				expectActions(currentRound.action().broadcastPrevote(&proposalValue))
	...
	t.Run(
		"Line 22: valid proposal with lockedValue matching proposal should prevote for proposal",
		func(t *testing.T) {
			stateMachine := setupStateMachine(t, 4, 3)
			...
			previousRound.start()
			previousRound.validator(0).
				proposal(expectedValue, -1).
				expectActions(previousRound.action().broadcastPrevote(&expectedValue))
			...
			nextRound.validator(1).
				proposal(expectedValue, -1).
				expectActions(nextRound.action().broadcastPrevote(&expectedValue))
			assertState(t, stateMachine, types.Height(0), types.Round(1), types.StepPrevote)
		},
	)

	t.Run(
		"Line 22: valid proposal with lockedValue not matching proposal should prevote nil",
		func(t *testing.T) {
			stateMachine := setupStateMachine(t, 4, 3)
			previousRound := newTestRound(t, stateMachine, 0, 0)
			nextRound := newTestRound(t, stateMachine, 0, 1)
			...
			nextRound.validator(1).
				proposal(value(43), -1).
				expectActions(nextRound.action().broadcastPrevote(nil))
			assertState(t, stateMachine, types.Height(0), types.Round(1), types.StepPrevote)
		},
	)
	...
	t.Run(
		"Line 22: proposal received when not in propose step should do nothing",
		func(t *testing.T) {
			stateMachine := setupStateMachine(t, 4, 3)
			currentRound := newTestRound(t, stateMachine, 0, 0)
