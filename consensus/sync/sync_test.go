...
func getTestData(
	...
	proposerAddr := validators.Proposer(height, consensusSync.RoundPlaceholder)
	blockBody := sync.BlockBody{
		Block: &core.Block{
			Header: &core.Header{
				...
				TransactionCount: uint64(rand.Uint64N(100)),
				EventCount:       uint64(rand.Uint64N(1000)),
	...
	proposal := starknet.Proposal{
		MessageHeader: types.MessageHeader[starknet.Address]{
			Height: height,
			Round:  consensusSync.RoundPlaceholder,
			Sender: proposerAddr,
		},
