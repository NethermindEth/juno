...
func TestAggregatedBloomFilter_UnmarshalBinary_Corrupt(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr: core.ErrBloomFilterRowBitsetLenMismatch,
			lazyRow: true,
		},
		{
			...
			wantErr: core.ErrBloomFilterRowBlobLenMismatch,
			lazyRow: true,
		},
		{
			name:    "truncated header",
			corrupt: data[:10],
			wantErr: core.ErrBloomFilterHeaderTooShort,
		},
		{
			name:    "truncated mid-row",
			corrupt: data[:30],
			wantErr: core.ErrBloomFilterLenMismatch,
		},
		{
			...
			wantErr: core.ErrBloomFilterRowCountMismatch,
		},
		{
			...
			wantErr: core.ErrBloomFilterRangeMismatch,
		},
		{
			name:    "trailing bytes",
