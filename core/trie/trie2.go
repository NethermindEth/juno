// Package trie implements a dense Merkle Patricia Trie2. See the documentation on [Trie2] for details.
package trie
...
// Trie2 is a dense Merkle Patricia Trie2 (i.e., all internal nodes have two children).
//
...
//
// The state trie [specification] describes a sparse Merkle Trie2.
...
type NewTrieFunc2 func(*Storage2, uint8) (*Trie2, error)

func NewTriePedersen2(storage *Storage2, height uint8) (*Trie2, error) {
	return newTrie2(storage, height, crypto.Pedersen)
}

func NewTriePoseidon2(storage *Storage2, height uint8) (*Trie2, error) {
	return newTrie2(storage, height, crypto.Poseidon)
}

func newTrie2(storage *Storage2, height uint8, hash crypto.HashFn) (*Trie2, error) {
	if height > felt.Bits {
...
// RunOnTempTriePedersen creates an in-memory Trie2 of height `height` and runs `do` on that Trie2
func RunOnTempTriePedersen2(height uint8, do func(*Trie2) error) error {
	trie, err := NewTriePedersen2(newMemStorage2(), height)
	if err != nil {
		return err
	}
	return do(trie)
...
func RunOnTempTriePoseidon2(height uint8, do func(*Trie2) error) error {
	trie, err := NewTriePoseidon2(newMemStorage2(), height)
	if err != nil {
		return err
	}
	return do(trie)
...
// FeltToKey converts a key, given in felt, to a trie.Key which when followed on a [Trie2],
// leads to the corresponding [Node]
func (t *Trie2) FeltToKey(k *felt.Felt) BitArray {
	var ba BitArray
...
// of the Trie2 to the node which is given by the key.
// The [storageNode]s are returned in descending order beginning with the root.
func (t *Trie2) nodesFromRoot(key *BitArray) ([]StorageNode, error) {
...
func (t *Trie2) Get(key *felt.Felt) (*felt.Felt, error) {
...
func (t *Trie2) GetNodeFromKey(key *BitArray) (*Node, error) {
	return t.storage.Get(key)
}

// check if we are updating an existing leaf, if yes avoid traversing the trie
func (t *Trie2) updateLeaf(nodeKey BitArray, node *Node, value *felt.Felt) (*felt.Felt, error) {
...
func (t *Trie2) handleEmptyTrie(old felt.Felt, nodeKey BitArray, node *Node, value *felt.Felt) (*felt.Felt, error) {
	if value.IsZero() {
...
func (t *Trie2) deleteExistingKey(sibling StorageNode, nodeKey BitArray, nodes []StorageNode) (*felt.Felt, error) {
...
func (t *Trie2) replaceLinkWithNewParent(key *BitArray, commonKey BitArray, siblingParent StorageNode) {
...
func (t *Trie2) insertOrUpdateValue(
	nodeKey *BitArray,
	node *Node,
	nodes []StorageNode,
	sibling StorageNode,
...
func (t *Trie2) Put(key, value *felt.Felt) (*felt.Felt, error) {
...
// Put updates the corresponding `value` for a `key`
func (t *Trie2) PutWithProof(key, value *felt.Felt, proof []*StorageNode) (*felt.Felt, error) {
	if key.Cmp(t.maxKey) > 0 {
		return nil, fmt.Errorf("key %s exceeds trie height %d", key, t.height)
	}
...
// Put updates the corresponding `value` for a `key`
func (t *Trie2) PutInner(key *BitArray, node *Node) error {
...
func (t *Trie2) setRootKey(newRootKey *BitArray) {
	t.rootKey = newRootKey
	t.rootKeyIsDirty = true
}

func (t *Trie2) updateValueIfDirty(key *BitArray) (*Node, error) { //nolint:gocyclo
...
func (t *Trie2) updateChildTriesSerially(root *Node, leftIsProof, rightIsProof bool) (*Node, *Node, error) {
	var leftChild, rightChild *Node
	var err error
	if !leftIsProof {
...
func (t *Trie2) updateChildTriesConcurrently(root *Node, leftIsProof, rightIsProof bool) (*Node, *Node, error) {
	var leftChild, rightChild *Node
	var lErr, rErr error
...
func (t *Trie2) deleteLast(nodes []StorageNode) error {
...
// Root returns the commitment of a [Trie2]
func (t *Trie2) Root() (*felt.Felt, error) {
...
func (t *Trie2) Commit() error {
	_, err := t.Root()
	return err
}

// RootKey returns db key of the [Trie2] root node
func (t *Trie2) RootKey() *BitArray {
	return t.rootKey
}
...
func (t *Trie2) HashFn() crypto.HashFn {
	return t.hash
}

// Try to print a [Trie2] in a somewhat human-readable form
/*
...
*/
func (t *Trie2) dump(level int, parentP *BitArray) {
	if t.rootKey == nil {
