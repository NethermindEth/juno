...
func (t *Trie2) Prove(key *felt.Felt, proof *ProofNodeSet) error {
	k := t.FeltToKey(key)
	...
	for i, sNode := range nodesFromRoot {
		sNodeEdge, sNodeBinary, err := storageNodeToProofNode2(t, parentKey, sNode)
		if err != nil {
			return err
		}
...
func (t *Trie2) GetRangeProof(leftKey, rightKey *felt.Felt, proofSet *ProofNodeSet) error {
...
func VerifyRangeProof2(root, first *felt.Felt, keys, values []*felt.Felt, proof *ProofNodeSet) (bool, error) { //nolint:funlen,gocyclo
...
// Juno's Trie2 has nodes that are Binary AND Edge, whereas the protocol requires nodes that are Binary XOR Edge.
// We need to convert the former to the latter for proof generation.
func storageNodeToProofNode2(tri *Trie2, parentKey *BitArray, sNode StorageNode) (*Edge, *Binary, error) {
	var edge *Edge
...
func buildTrie2(height uint8, rootKey *BitArray, nodes []*StorageNode, keys, values []*felt.Felt) (*Trie2, error) {
	tr, err := NewTriePedersen2(newMemStorage2(), height)
	if err != nil {
		return nil, err
	}
