...
func TestVerifyTransactionHash(t *testing.T) {
	...
	txnHash0 := utils.HexToFelt(
		t,
		"0x1b4d9f09276629d496af1af8ff00173c11ff146affacb1b5c858d7aa89001ae",
	)
	txn0, err := gw.Transaction(t.Context(), txnHash0)
	require.NoError(t, err)

	txnHash1 := utils.HexToFelt(
		t,
	...
	txnHash4 := utils.HexToFelt(
		t,
		"0x2897e3cec3e24e4d341df26b8cf1ab84ea1c01a051021836b36c6639145b497",
	)
	...
	t.Run("contains bad transaction", func(t *testing.T) {
		...
		*badTxn1 = *txn3.(*core.L1HandlerTransaction)
		...
		tests := map[felt.Felt]struct {
			...
			*badTxn0.Hash(): {
				...
				wantErr: fmt.Errorf(
					...
					badTxn0.Hash().String(),
					errors.New(
						"invalid Transaction (type: *core.DeclareTransaction) version: 0x12",
					),
				),
				txn: badTxn0,
			},
			*badTxn1.Hash(): {
				...
				wantErr: fmt.Errorf(
					"cannot verify transaction hash of Transaction %v",
					badTxn1.Hash().String(),
				),
				txn: badTxn1,
			},
		...
		for _, test := range tests {
			t.Run(test.name, func(t *testing.T) {
				tErr := core.VerifyTransactions(
					[]core.Transaction{test.txn},
					&utils.Mainnet,
					"99.99.99",
				)
	...
	tests := []struct {
		...
		},
		{
			...
			tx: &core.L1HandlerTransaction{
				ContractAddress: utils.HexToFelt(
					t,
					"0x078466c2444176f0be70650b3d1f520e19a095ca5fa6ff124ddc49f27a30bdac",
				),
				EntryPointSelector: utils.HexToFelt(
					t,
