...
func TestPendingData_PendingState(t *testing.T) {
	...
	t.Run("pre-confirmed - state with storage update", func(t *testing.T) {
		preConfirmedSimple := core.PreConfirmed{
			StateUpdate: &core.StateUpdate{
				StateDiff: &stateDiff,
			},
		}
		preConfirmedSimple.Block = &emptyBlock

		state := preConfirmedSimple.PendingState(nil)
