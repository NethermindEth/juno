...
func TestUpdate(t *testing.T) {
  ...
  "state_diff": {
	...
	coreUpdate.StateDiff.StorageDiffs = make(map[string][]core.KVPair)
	for addr, diffs := range gatewayUpdate.StateDiff.StorageDiffs {
		for _, diff := range diffs {
			coreUpdate.StateDiff.StorageDiffs[addr] = append(coreUpdate.StateDiff.StorageDiffs[addr], core.KVPair{
				Key:   diff.Key,
				Value: diff.Value,
			})
		}
