...
func TestBlakeHash(t *testing.T) {
	...
	t.Run("empty array", func(t *testing.T) {
		actual, err := blake2s.Blake2sArray[felt.Felt]()
		require.NoError(t, err)
	...
	t.Run("small felt", func(t *testing.T) {
		actual, err := blake2s.Blake2sArray(felt.NewFromUint64[felt.Felt](1<<63 - 1))
		require.NoError(t, err)
	...
	t.Run("at boundary", func(t *testing.T) {
		actual, err := blake2s.Blake2sArray(felt.NewFromUint64[felt.Felt](1 << 63))
		require.NoError(t, err)
	...
	t.Run("large felt", func(t *testing.T) {
		actual, err := blake2s.Blake2sArray(
	...
	t.Run("max u64", func(t *testing.T) {
		actual, err := blake2s.Blake2sArray(felt.NewFromUint64[felt.Felt](^uint64(0)))
		require.NoError(t, err)
