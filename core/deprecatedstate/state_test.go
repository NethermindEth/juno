...
func TestUpdate(t *testing.T) {
	...
	t.Run("error when state current root doesn't match state update's old root",
		func(t *testing.T) {
			...
			require.EqualError(t, state.Update(&core.Header{Number: 1}, su, nil, false), expectedErr)
		})
	...
	t.Run("update systemContracts storage", func(t *testing.T) {
		require.NoError(t, state.Update(&core.Header{Number: 4}, su4, nil, false))
...
func TestContractClassHash(t *testing.T) {
	...
	t.Run("replace class hash", func(t *testing.T) {
		...
		require.NoError(t, state.Update(&core.Header{Number: 2}, replaceUpdate, nil, false))
...
func TestNonce(t *testing.T) {
	...
	require.NoError(t, state.Update(&core.Header{Number: 0}, su, nil, false))
	...
	t.Run("update contract nonce", func(t *testing.T) {
		...
		require.NoError(t, state.Update(&core.Header{Number: 1}, su, nil, false))
...
func TestStateHistoricalReads(t *testing.T) {
	...
	t.Run("should return an error for a location that changed on the given height", func(t *testing.T) {
		...
		assert.ErrorIs(t, err, deprecatedstate.ErrCheckHeadState)
...
func TestHistory(t *testing.T) {
	...
	for desc, test := range map[string]struct {
		...
		"contract storage": {
			logger: func(txn db.KeyValueWriter, location, oldValue *felt.Felt, height uint64) error {
				return deprecatedstate.WriteContractStorageHistory(
					txn,
					contractAddress,
					location,
					oldValue,
			...
			deleter: func(txn db.KeyValueWriter, location *felt.Felt, height uint64) error {
				return deprecatedstate.DeleteContractStorageHistory(txn, contractAddress, location, height)
			},
		},
		"contract nonce": {
			logger:  deprecatedstate.WriteContractNonceHistory,
			getter:  state.ContractNonceAt,
			deleter: deprecatedstate.DeleteContractNonceHistory,
		},
		"contract class hash": {
			logger:  deprecatedstate.WriteContractClassHashHistory,
			getter:  state.ContractClassHashAt,
			deleter: deprecatedstate.DeleteContractClassHashHistory,
		},
	} {
...
func TestRevert(t *testing.T) {
	...
	t.Run("revert a replaced class", func(t *testing.T) {
		...
		require.NoError(t, state.Update(&core.Header{Number: 2}, replaceStateUpdate, nil, false))
		require.NoError(t, state.Revert(&core.Header{Number: 2}, replaceStateUpdate))
	...
	t.Run("revert a nonce update", func(t *testing.T) {
		...
		require.NoError(t, state.Update(&core.Header{Number: 2}, nonceStateUpdate, nil, false))
		require.NoError(t, state.Revert(&core.Header{Number: 2}, nonceStateUpdate))
	...
	t.Run("revert declared classes", func(t *testing.T) {
		...
		deprecatedCairo := &core.DeprecatedCairoClass{
			...
			Externals: []core.DeprecatedEntryPoint{
				{
					Selector: felt.NewFromBytes[felt.Felt]([]byte("e1")),
					Offset:   felt.NewFromBytes[felt.Felt]([]byte("e2")),
				},
				{
	...
	t.Run("should be able to revert all the state", func(t *testing.T) {
		require.NoError(t, state.Revert(&core.Header{Number: 2}, su2))
		root, err := state.Commitment("")
		require.NoError(t, err)
		require.Equal(t, su2.OldRoot, &root)
		require.NoError(t, state.Revert(&core.Header{Number: 1}, su1))
		root, err = state.Commitment("")
		require.NoError(t, err)
		require.Equal(t, su1.OldRoot, &root)
		require.NoError(t, state.Revert(&core.Header{Number: 0}, su0))
		root, err = state.Commitment("")
		require.NoError(t, err)
...
func TestRevertGenesisStateDiff(t *testing.T) {
	...
	require.NoError(t, state.Update(&core.Header{Number: 0}, su, nil, false))
	require.NoError(t, state.Revert(&core.Header{Number: 0}, su))
}
...
func TestRevertDeclaredClasses(t *testing.T) {
	...
	require.NoError(t, state.Update(&core.Header{Number: 0}, declareDiff, newClasses, false))
	...
	require.NoError(t, state.Update(&core.Header{Number: 1}, declareDiff, newClasses, false))
	...
	require.NoError(t, state.Revert(&core.Header{Number: 1}, declareDiff))
	...
	require.NoError(t, state.Revert(&core.Header{Number: 0}, declareDiff))
...
func TestRevertMigratedCasmClasses(t *testing.T) {
	...
	require.NoError(t, state.Update(&core.Header{Number: 0}, su0, classes, true))
	...
	require.NoError(t, state.Update(&core.Header{Number: 1}, su1, nil, true))
...
func TestCommitmentV014AlwaysPoseidon(t *testing.T) {
	// Regression test for v0.14+ state commitment formula.
	// Scenario: single storage write to system contract 0x2, no declared classes.
	// v0.14+ must apply Poseidon even when class_commitment == 0.
	testDB := memory.New()
	txn := testDB.NewIndexedBatch()
	...
	require.NoError(t, state.Update(&core.Header{Number: 0, ProtocolVersion: "0.14.2"}, su, nil, true))

	// Pre-0.14 formula would return raw storageRoot (the bug)
	pre014Root, err := state.Commitment("")
	require.NoError(t, err)
	assert.False(t, pre014Root.IsZero())

	// v0.14 formula must apply Poseidon even when classesRoot == 0
	v014Root, err := state.Commitment("0.14.2")
	require.NoError(t, err)

	assert.NotEqual(t, pre014Root, v014Root,
		"v0.14 commitment must differ from pre-0.14 when classesRoot is zero")

	// Verify against Pathfinder's known-good value
	expectedRoot := felt.NewUnsafeFromString[felt.Felt](
