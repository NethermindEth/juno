...
func TestDatabase(t *testing.T) {
	...
	t.Run("Commit handles concurrent operations", func(t *testing.T) {
		...
		for i := range numTries {
			...
			tries[i] = struct {
				...
				root:   *rootHash,
				parent: *felt.NewFromUint64[felt.Hash](uint64(i*100 - 1)),
				classNodes: map[trieutils.Path]trienode.TrieNode{
					rootPath: rootNode,
					leafPath: leafNode,
				},
				classRoot:    *rootHash,
				contractRoot: *felt.NewFromUint64[felt.Hash](uint64(3000 + i)),
			}
	...
	t.Run("Update and Commit with deleted nodes", func(t *testing.T) {
		...
		newRootHash := felt.NewFromUint64[felt.Hash](101)
		newRootNode := trienode.NewNonLeaf(
			felt.Felt(*newRootHash),
			createBinaryNodeBlob(&felt.Zero, leaf2Hash),
		)
