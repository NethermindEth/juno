...
func layoutName(l txlayout.TransactionLayout) string {
	switch l {
	case txlayout.TransactionLayoutPerTx:
		return "PerTx"
	case txlayout.TransactionLayoutCombined:
		return "Combined"
	default:
		return "Unknown"
	}
...
func runTestTransactionLayout(
	...
	t.Run(layoutName(layout), func(t *testing.T) {
		...
		t.Run("NonExistentBlock_TransactionsByBlockNumber", func(t *testing.T) {
			if layout == txlayout.TransactionLayoutPerTx {
		...
		t.Run("DeleteTxsAndReceipts", func(t *testing.T) {
			// Delete all blocks
			for blockNum := range chainHeight {
				//nolint:staticcheck // SA1019: needed since it's used by old state
...
func TestTransactionLayout(t *testing.T) {
	...
	runTestTransactionLayout(t, transactions, receipts, headers, txlayout.TransactionLayoutPerTx)
	runTestTransactionLayout(t, transactions, receipts, headers, txlayout.TransactionLayoutCombined)
}
...
func TestTransactionLayout_UnknownLayout(t *testing.T) {
	...
	unknownLayout := txlayout.TransactionLayout(99)

	t.Run("TransactionByBlockAndIndex", func(t *testing.T) {
		_, err := unknownLayout.TransactionByBlockAndIndex(database, 0, 0)
		require.ErrorIs(t, err, txlayout.ErrUnknownTransactionLayout)
	})
