...
func newIngestor(
	...
	for i := range tasks {
		tasks[i] = task{
			batch:           batchSemaphore.GetBlocking(),
			totalTxCount:    0,
			totalBlockCount: 0,
		}
	}
...
func (c *ingestor) Run(index int, startBlock uint64, outputs chan<- task) error {
	if startBlock%logRate == 0 {
		c.logger.Info("Migrating block", zap.Uint64("startBlock", startBlock))
	}
	...
	if t.batch.Size() >= targetBatchByteSize {
		outputs <- *t
		*t = task{
			batch:           c.batchSemaphore.GetBlocking(),
			totalTxCount:    0,
			totalBlockCount: 0,
		}
		return nil
...
func (c *ingestor) validateCount(
	...
	if fetchedTxCount == 0 || fetchedReceiptCount == 0 {
		...
		if has {
			c.logger.Info(
				"skipping already migrated block",
...
) iter.Seq2[cbor.RawMessage, error] {
