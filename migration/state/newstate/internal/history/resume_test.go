...
// source writes a refused address into progress, so an interrupted walk leaves
// progress on the first address never handed out.
func TestAddressSeqEscapesResumePoint(t *testing.T) {
	...
	require.Equal(t, felt.Address(addrs[1]), progress, "the refused address is the resume point")
	...
	// a walk that runs out leaves progress where it started.
	seq, sourceErr = addressSeq(memDB, &progress)
	...
	require.Equal(
		t, felt.Address(addrs[1]), progress, "a completed walk leaves progress untouched",
	)
}
...
func TestRunPhaseDrainsHandedOutContractsOnCancel(t *testing.T) {
	...
	for i := range addrs {
		...
		if bytes.Compare(addrBytes[:], progressBytes[:]) >= 0 {
			break
		}
		handedOut++
