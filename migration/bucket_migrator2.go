...
var _ Migration2 = (*BucketMigrator2)(nil)

type (
	BucketMigratorDoFunc2 func(t db.KeyValueWriter, b1, b2 []byte, n *utils.Network) error
	// BucketMigratorKeyFilter func([]byte) (bool, error)
)

type BucketMigrator2 struct {
	target db.Bucket
	...
	do        BucketMigratorDoFunc2

	// key to seek to when starting the migration
	startFrom []byte
}

func NewBucketMigrator2(target db.Bucket, do BucketMigratorDoFunc2) *BucketMigrator2 {
	return &BucketMigrator2{
		target:    target,
...
func NewBucketMover2(source, destination db.Bucket) *BucketMigrator2 {
	return NewBucketMigrator2(source, func(txn db.KeyValueWriter, key, value []byte, n *utils.Network) error {
		err := txn.Delete(key)
		if err != nil {
			return err
		}
...
func (m *BucketMigrator2) WithBatchSize(batchSize uint) *BucketMigrator2 {
	m.batchSize = batchSize
	return m
}

func (m *BucketMigrator2) WithBefore(before func()) *BucketMigrator2 {
	m.before = before
	return m
}

func (m *BucketMigrator2) WithKeyFilter(keyFilter BucketMigratorKeyFilter) *BucketMigrator2 {
	m.keyFilter = keyFilter
	return m
}

func (m *BucketMigrator2) Before(_ []byte) error {
	m.before()
	return nil
}

func (m *BucketMigrator2) Migrate(ctx context.Context, txn db.IndexedBatch, network *utils.Network, log utils.SimpleLogger) ([]byte, error) {
