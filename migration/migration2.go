...
type MigrationFunc2 func(db.IndexedBatch, *utils.Network) error

// Migrate returns f(txn).
func (f MigrationFunc2) Migrate(_ context.Context, txn db.IndexedBatch, network *utils.Network, _ utils.SimpleLogger) ([]byte, error) {
	return nil, f(txn, network)
}

// Before is a no-op.
func (f MigrationFunc2) Before(_ []byte) error { return nil }
...
var defaultMigrations2 = []Migration2{
	MigrationFunc2(migration00002),
	MigrationFunc2(relocateContractStorageRootKeys2),
	MigrationFunc2(recalculateBloomFilters2),
	new(changeTrieNodeEncoding2),
	MigrationFunc2(calculateBlockCommitments2),
	NewBucketMigrator2(db.ClassesTrie, migrateTrieRootKeysFromBitsetToTrieKeys2).WithKeyFilter(rootKeysFilter(db.ClassesTrie)),
	NewBucketMigrator2(db.StateTrie, migrateTrieRootKeysFromBitsetToTrieKeys2).WithKeyFilter(rootKeysFilter(db.StateTrie)),
	NewBucketMigrator2(db.ContractStorage, migrateTrieRootKeysFromBitsetToTrieKeys2).WithKeyFilter(rootKeysFilter(db.ContractStorage)),
	NewBucketMigrator2(db.ClassesTrie, migrateTrieNodesFromBitsetToTrieKey2(db.ClassesTrie)).WithKeyFilter(nodesFilter(db.ClassesTrie)),
	NewBucketMover2(db.Temporary, db.ClassesTrie),
	NewBucketMigrator2(db.StateTrie, migrateTrieNodesFromBitsetToTrieKey2(db.StateTrie)).WithKeyFilter(nodesFilter(db.StateTrie)),
	NewBucketMover2(db.Temporary, db.StateTrie),
	NewBucketMigrator2(db.ContractStorage, migrateTrieNodesFromBitsetToTrieKey2(db.ContractStorage)).
		WithKeyFilter(nodesFilter(db.ContractStorage)),
	NewBucketMover2(db.Temporary, db.ContractStorage),
	NewBucketMigrator2(db.StateUpdatesByBlockNumber, changeStateDiffStruct2).WithBatchSize(100), //nolint:mnd
	NewBucketMigrator2(db.Class, migrateCairo1CompiledClass2).WithBatchSize(1_000),              //nolint:mnd
	MigrationFunc2(calculateL1MsgHashes2),
	MigrationFunc2(removePendingBlock2),
}

// var ErrCallWithNewTransaction = errors.New("call with new transaction")

func MigrateIfNeeded2(ctx context.Context, targetDB db.KeyValueStore, network *utils.Network, log utils.SimpleLogger) error {
	return migrateIfNeeded2(ctx, targetDB, network, log, defaultMigrations2)
}

func migrateIfNeeded2(ctx context.Context, targetDB db.KeyValueStore, network *utils.Network, log utils.SimpleLogger, migrations []Migration2) error {
	/*
...
func migration00002(txn db.IndexedBatch, _ *utils.Network) error {
...
func relocateContractStorageRootKeys2(txn db.IndexedBatch, _ *utils.Network) error {
	it, err := txn.NewIterator(nil, false)
	if err != nil {
		return err
	}
...
func recalculateBloomFilters2(txn db.IndexedBatch, _ *utils.Network) error {
...
type changeTrieNodeEncoding2 struct {
...
func (m *changeTrieNodeEncoding2) Before(_ []byte) error {
...
func (m *changeTrieNodeEncoding2) Migrate(_ context.Context, txn db.IndexedBatch, _ *utils.Network, _ utils.SimpleLogger) ([]byte, error) {
...
func calculateBlockCommitments2(txn db.IndexedBatch, network *utils.Network) error {
...
func migrateTrieRootKeysFromBitsetToTrieKeys2(txn db.KeyValueWriter, key, value []byte, _ *utils.Network) error {
...
func migrateTrieNodesFromBitsetToTrieKey2(target db.Bucket) BucketMigratorDoFunc2 {
