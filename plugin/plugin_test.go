...
func TestPlugin(t *testing.T) {
	...
	synchronizer := sync.New(bc, integGw, utils.NewNopZapLogger(), 0, 0, false, nil).WithPlugin(plugin)
	...
	t.Run("resync to mainnet with the same db", func(t *testing.T) {
		...
		synchronizer = sync.New(bc, mainGw, utils.NewNopZapLogger(), 0, 0, false, nil).WithPlugin(plugin)
		ctx, cancel = context.WithTimeout(t.Context(), timeout)
		require.NoError(t, synchronizer.Run(ctx))
		cancel()
