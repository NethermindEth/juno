...
func MarshalFelt[F FeltLike](value *F) ([]byte, error) {
	data := make([]byte, maxCBORFeltLen)
	n := encodeLimbs(data, value)
	return data[:n], nil
}

func UnmarshalFelt[F FeltLike](data []byte, value *F) error {
	if decodeFelt(data, value) {
		return nil
	}

	// An array has no unmarshal hook, so the decoder cannot recurse into this function.
	// A CBOR null decodes as a no-op, so generic carries the current value in.
	generic := [Limbs]uint64(*value)
