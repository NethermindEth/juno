...
func MarshalFeltSlice[F FeltLike](slice []F) ([]byte, error) {
	if slice == nil {
...
// UnmarshalFeltSlice is the body of felt.Slice's UnmarshalCBOR.
func UnmarshalFeltSlice[F FeltLike](data []byte, slice *[]F) error {
	size, offset, ok := decodeCBORArrayHeader(data)
	if !ok {
		return unmarshalFeltSliceGeneric(data, slice)
	}
...
// unmarshalFeltSliceGeneric handles any shape the fast path does not recognise.
func unmarshalFeltSliceGeneric[F FeltLike](data []byte, slice *[]F) error {
	var buffer []F
	if err := codec.unmarshal(data, &buffer); err != nil {
		return err
	}
