...
func TestUnreliableSubscription(t *testing.T) {
	...
	require.NoError(t, err)
	t.Cleanup(eth.Close)
	eth.WithMaxResubscribeWaitTime(time.Millisecond)
...
type wrongChainIDHandler struct {
	h *ethHandler
}

func (e *wrongChainIDHandler) ChainId() *hexutil.Big { //nolint:stylecheck
	wrongID := new(big.Int).Add(e.h.ChainId().ToInt(), new(big.Int).SetInt64(1))
	return (*hexutil.Big)(wrongID)
}

func (e *wrongChainIDHandler) GetBlockByNumber(num rpc.BlockNumber, withTxs bool) (map[string]any, error) {
	return e.h.GetBlockByNumber(num, withTxs)
}

func (e *wrongChainIDHandler) Logs(ctx context.Context, crit filters.FilterCriteria) (*rpc.Subscription, error) {
	return e.h.Logs(ctx, crit)
}

func TestMismatchedChainID(t *testing.T) {
	network := utils.MAINNET
	h := &wrongChainIDHandler{
		h: &ethHandler{
			t:       t,
			network: network,
		},
	...
	require.NoError(t, err)
	t.Cleanup(eth.Close)
	sub := eth.WatchL1Heads(context.Background(), make(chan<- *core.L1Head))
	require.ErrorContains(t, <-sub.Err(), "mismatched L1 and L2 networks")
}

type ethRPCHandler interface {
	ChainId() *hexutil.Big
	GetBlockByNumber(num rpc.BlockNumber, withTxs bool) (map[string]any, error)
	Logs(ctx context.Context, crit filters.FilterCriteria) (*rpc.Subscription, error)
}

func setupServer(t *testing.T, h ethRPCHandler) string {
	rpcServer := rpc.NewServer()
	require.NoError(t, rpcServer.RegisterName("eth", h))
