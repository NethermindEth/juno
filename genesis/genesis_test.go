...
func TestGenesisStateDiff(t *testing.T) {
	...
	t.Run("empty genesis config", func(t *testing.T) {
		...
		_, _, err := genesis.GenesisStateDiff(genesisConfig, chain, vm.New(log), &network, log)
		require.NoError(t, err)
	})
	...
	t.Run("valid non-empty genesis config", func(t *testing.T) {
		...
		stateDiff, newClasses, err := genesis.GenesisStateDiff(genesisConfig, chain, vm.New(log), &network, log)
		require.NoError(t, err)
