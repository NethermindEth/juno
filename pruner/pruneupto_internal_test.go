...
func TestPruneUpto_NoOp(t *testing.T) {
	t.Run("empty database returns 0", func(t *testing.T) {
		database := testutils.NewPebbleTestDB(t)
		pruned, oldestKept, err := pruneUpto(t.Context(), database, 20, testTargetBatchByteSize)
		require.NoError(t, err)
...
func TestPruneUpto_ResumeAfterMidwayCancel(t *testing.T) {
	...
	pruned1, oldestKept1, err := pruneUpto(ctx, wrapped, endExclusive, testTargetBatchByteSize)
	require.NoError(t, err)
...
func TestPruneUpto_RollingCarveOut(t *testing.T) {
	...
	pruned, oldestKept, err = pruneUpto(t.Context(), database, 30, testTargetBatchByteSize)
	require.NoError(t, err)
