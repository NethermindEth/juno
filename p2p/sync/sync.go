	...
	loop:
		for res := range headersIt {
			...
			case *header.BlockHeadersResponse_Fin:
				break loop
			default:
				s.log.Warn("Unexpected HeaderMessage from getBlockHeaders", zap.Any("v", v))
				break loop
			}
	...
	loop:
		for res := range classesIt {
			...
			case *syncclass.ClassesResponse_Fin:
				break loop
			default:
				s.log.Warn("Unexpected ClassMessage from getClasses", zap.Any("v", v))
				break loop
			}
		}
	...
	loop:
		for res := range stateDiffsIt {
			...
			case *state.StateDiffsResponse_Fin:
				break loop
			default:
				s.log.Warn("Unexpected ClassMessage from getStateDiffs", zap.Any("v", v))
				break loop
			}
		}
	...
	loop:
		for res := range eventsIt {
			...
			case *event.EventsResponse_Fin:
				break loop
			default:
				s.log.Warn("Unexpected EventMessage from getEvents", zap.Any("v", v))
				break loop
			}
		}
	...
	loop:
		for res := range txsIt {
			...
			case *synctransaction.TransactionsResponse_Fin:
				break loop
			default:
				s.log.Warn("Unexpected TransactionMessage from getTransactions", zap.Any("v", v))
				break loop
			}
		}
...
func randomPeer(host host.Host, log utils.StructuredLogger) peer.ID {
	...
	log.Debug("Random chosen peer's info", zap.Any("peerInfo", store.PeerInfo(p)))
