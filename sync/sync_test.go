...
func TestSyncBlocks(t *testing.T) {
	...
	t.Run("sync multiple blocks in an empty db", func(t *testing.T) {
		...
		synchronizer := sync.New(bc, gw, sync.NopObserver{}, log, time.Duration(0))
	...
	t.Run("sync multiple blocks, with an unreliable gw", func(t *testing.T) {
		...
		synchronizer := sync.New(bc, mockSNData, sync.NopObserver{}, log, time.Duration(0))
...
func TestReorg(t *testing.T) {
	...
	synchronizer := sync.New(bc, integGw, sync.NopObserver{}, utils.NewNopZapLogger(), time.Duration(0))
	...
	t.Run("resync to mainnet with the same db", func(t *testing.T) {
		...
		synchronizer = sync.New(bc, mainGw, sync.NopObserver{}, utils.NewNopZapLogger(), time.Duration(0))
		ctx, cancel = context.WithTimeout(context.Background(), timeout)
		require.NoError(t, synchronizer.Run(ctx))
		cancel()
...
func TestPending(t *testing.T) {
	...
	log := utils.NewNopZapLogger()
	bc := blockchain.New(testDB, utils.MAINNET, log)
	synchronizer := sync.New(bc, gw, sync.NopObserver{}, log, time.Millisecond*100)
