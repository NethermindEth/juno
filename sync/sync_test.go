...
func TestReorg(t *testing.T) {
	...
	synchronizer := sync.New(bc, integGw, utils.NewNopZapLogger(), time.Duration(0), false, testDB)
	...
	t.Run("resync to mainnet with the same db", func(t *testing.T) {
		...
		synchronizer = sync.New(bc, mainGw, utils.NewNopZapLogger(), time.Duration(0), false, testDB)
		ctx, cancel = context.WithTimeout(context.Background(), timeout)
		require.NoError(t, synchronizer.Run(ctx))
		cancel()
...
func TestSubscribeNewHeads(t *testing.T) {
	...
	chain := blockchain.New(testDB, &integration, nil)
