...
func TestPollerColdBootstrapNoGap(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		view := h.storage.SnapshotForHead(bottomAbove(h.head.Number))
...
func TestPollerSameHeightNoBackfill(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		h := wirePoller(t, fx.bc, fx.head, ds)
		_, err := h.storage.ApplyUpdate(seed, h.head.Number+1, 0, bottomAbove(h.head.Number))
		require.NoError(t, err)
		before := h.storage.SnapshotForHead(bottomAbove(h.head.Number))
		...
		after := h.storage.SnapshotForHead(bottomAbove(h.head.Number))
...
func TestPollerMultiTickExtendsChain(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		time.Sleep(tickInterval)
		synctest.Wait()
		view1 := h.storage.SnapshotForHead(bottomAbove(h.head.Number))
		assertChain(t, &view1, entry(1, &block1))

		// Tick 2.
		time.Sleep(tickInterval)
		synctest.Wait()
		view2 := h.storage.SnapshotForHead(bottomAbove(h.head.Number))
		assertChain(t, &view2,
			entry(1, &block1),
			entry(2, &block2),
		)
		...
		time.Sleep(tickInterval)
		synctest.Wait()
		view3 := h.storage.SnapshotForHead(bottomAbove(h.head.Number))
		assertChain(t, &view3,
			entry(1, &block1),
			entry(2, &block2),
			entry(3, &block3),
...
func TestPollerHeadAdvancesDropsCommittedEntries(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		h := wirePoller(t, fx.bc, fx.head, ds)
		_, err := h.storage.ApplyUpdate(seed1, h.head.Number+1, 0, bottomAbove(h.head.Number))
		require.NoError(t, err)
		_, err = h.storage.ApplyUpdate(seed2, h.head.Number+2, 0, bottomAbove(h.head.Number))
		require.NoError(t, err)
		before := h.storage.SnapshotForHead(bottomAbove(h.head.Number))
		...
		view := h.storage.SnapshotForHead(bottomAbove(newHead.Number))
...
func TestPollerReorgLowerHeightDifferentIdentifier(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		_, err = h.storage.ApplyUpdate(seed2, h.head.Number+2, 0, bottomAbove(h.head.Number))
		require.NoError(t, err)
		_, err = h.storage.ApplyUpdate(seed3, h.head.Number+3, 0, bottomAbove(h.head.Number))
		require.NoError(t, err)
		before := h.storage.SnapshotForHead(bottomAbove(h.head.Number))
