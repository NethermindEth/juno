package sync

import (
	"github.com/NethermindEth/juno/core"
	"github.com/NethermindEth/juno/core/felt"
	"github.com/NethermindEth/juno/core/trie"
)

// type Pending struct {
// 	Block       *core.Block
// 	StateUpdate *core.StateUpdate
// 	NewClasses  map[felt.Felt]core.Class
// }

type PendingState2 struct {
	stateDiff  *core.StateDiff
	newClasses map[felt.Felt]core.Class
	head2      core.StateReader2
}

func NewPendingState2(stateDiff *core.StateDiff, newClasses map[felt.Felt]core.Class, head core.StateReader2) *PendingState2 {
	return &PendingState2{
		stateDiff:  stateDiff,
		newClasses: newClasses,
		head2:      head,
	}
}

func (p *PendingState2) ContractClassHash(addr *felt.Felt) (*felt.Felt, error) {
	if classHash, ok := p.stateDiff.ReplacedClasses[*addr]; ok {
		return classHash, nil
	} else if classHash, ok = p.stateDiff.DeployedContracts[*addr]; ok {
		return classHash, nil
	}
	return p.head2.ContractClassHash(addr)
}

func (p *PendingState2) ContractNonce(addr *felt.Felt) (*felt.Felt, error) {
	if nonce, found := p.stateDiff.Nonces[*addr]; found {
		return nonce, nil
	} else if _, found = p.stateDiff.DeployedContracts[*addr]; found {
		return &felt.Felt{}, nil
	}
	return p.head2.ContractNonce(addr)
}

func (p *PendingState2) ContractStorage(addr, key *felt.Felt) (*felt.Felt, error) {
	if diffs, found := p.stateDiff.StorageDiffs[*addr]; found {
		if value, found := diffs[*key]; found {
			return value, nil
		}
	}
	if _, found := p.stateDiff.DeployedContracts[*addr]; found {
		return &felt.Felt{}, nil
	}
	return p.head2.ContractStorage(addr, key)
}

func (p *PendingState2) Class(classHash *felt.Felt) (*core.DeclaredClass, error) {
	if class, found := p.newClasses[*classHash]; found {
		return &core.DeclaredClass{
			At:    0,
			Class: class,
		}, nil
	}

	return p.head2.Class(classHash)
}

func (p *PendingState2) ClassTrie() (*trie.Trie2, error) {
	return nil, core.ErrHistoricalTrieNotSupported
}

func (p *PendingState2) ContractTrie() (*trie.Trie2, error) {
	return nil, core.ErrHistoricalTrieNotSupported
}

func (p *PendingState2) ContractStorageTrie(addr *felt.Felt) (*trie.Trie2, error) {
	return nil, core.ErrHistoricalTrieNotSupported
}
