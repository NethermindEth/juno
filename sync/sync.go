...
func (s *Synchronizer) fetcherTask(ctx context.Context, height uint64, verifiers *stream.Stream,
	...
	for {
		...
		default:
			...
			if err != nil {
				lastPossiblyValidHeight := s.isReverting(ctx, height)
				if lastPossiblyValidHeight != nil {
					return func() {
...
func (s *Synchronizer) isReverting(ctx context.Context, nextHeight uint64) (lastPossiblyValidHeight *uint64) {
	...
	case remoteHeight > localHeight:
		return nil
	// If the latest block is the same as the head, revert if the hash is different
	case remoteHeight == localHeight:
		if *remoteHead.Hash != *localHead.Hash {
			return utils.HeapPtr(remoteHeight - 1)
		}
		return nil
	// If the latest block is older than the head, revert is necessary regardless of the hash
	default:
		return &remoteHeight
	}
}
...
func (s *Synchronizer) verifierTask(
	...
	if err != nil {
		return func() {
			...
			s.log.Warnw("Sanity checks failed", "number", committedBlock.Block.Number, "hash", committedBlock.Block.Hash.ShortString(), "err", err)
			resetStreams()
		}
	}
...
func (s *Synchronizer) revertHead(localHeader *core.Header) {
	s.log.Infow("Reorg detected", "localHead", localHeader.Hash, "forkHead")
