...
func TestClassV1(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.Equal(t, compiled.Prime, "0x"+v1Class.Casm.Prime.Text(felt.Base16))
			assert.Equal(t, compiled.Bytecode, v1Class.Casm.Bytecode)
			assert.Equal(t, compiled.Hints, v1Class.Casm.Hints)
			assert.Equal(t, compiled.CompilerVersion, v1Class.Casm.CompilerVersion)
			assert.Equal(t, len(compiled.EntryPoints.External), len(v1Class.Casm.External))
			assert.Equal(t, len(compiled.EntryPoints.Constructor), len(v1Class.Casm.Constructor))
			assert.Equal(t, len(compiled.EntryPoints.L1Handler), len(v1Class.Casm.L1Handler))
