...
| block lag | iterations where the node's pre-confirmed block number was behind the sequencer's: the node had not yet fetched the new round (poll lag) or had not yet stored the closed block (sync lag) |
