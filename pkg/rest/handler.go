...
func NewServer(rest_port string, feeder_gateway string) *Server {
	...
	m.HandleFunc("/get_contract_addresses", rest_handler.GetBlockIDByHash)
	m.HandleFunc("/get_contract_addresses", rest_handler.GetTransactionHashByID)
	m.HandleFunc("/get_contract_addresses", rest_handler.GetTransactionIDByHash)
