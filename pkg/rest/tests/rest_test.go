...
// func TestRestGetContractAddress(t *testing.T) {
...
func TestGetBlockHandler(t *testing.T) {
	...
	//populate query args for http request
	rq := req.URL.Query()
	...
	blockNumber, ok_blockNumber := req.URL.Query()["blockNumber"]
	if !ok_blockNumber {
		t.Fatal("failed to create a query with arguments")
	}
