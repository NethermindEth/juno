...
func TestState(t *testing.T) {
	...
	const (
		contractHash = "10455c752b86932ce552f2b0fe81a880746649b9aee7e0d842bf3f52378f9f8"
		stateRoot    = "021870ba80540e7831fb21c591ee93481f5ae1bb71ff85a86ddd465be4eddee6"
	)
	...
	len := 251
	state := New(store.New(), len)
	for addr, diff := range addresses {
		storage := New(store.New(), len)
		...
		hash, _ := new(big.Int).SetString(contractHash, 16)
		val, _ := pedersen.Digest(hash, storage.Commitment())
		val, _ = pedersen.Digest(val, big.NewInt(0))
		val, _ = pedersen.Digest(val, big.NewInt(0))

		state.Put(key, val)
	}
	root, _ := new(big.Int).SetString(stateRoot, 16)
	got := state.Commitment()
	if root.Cmp(got) != 0 {
		t.Errorf("state.Commitment() = %x, want = %x", got, root)
	}
}
