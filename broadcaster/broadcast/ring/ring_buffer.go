...
// broadcast package; see [RingBuffer] for the sequence and slot protocol.
package ring
...
// of the slot they need (see [RingBuffer.Iterator] and [Slot.waitForSequence]),
// so a Write only wakes readers of the slot it filled.
//
// Fields:
...
//
// RingBuffer has no lifecycle of its own. Cancelling the ctx passed to [RingBuffer.Iterator] ends
...
// Tail returns the sequence of the last published message; 0 before the first [RingBuffer.Write].
...
// Iterator returns an [iter.Seq] yielding [EventOrLag] values in sequence order, or a lag
// notification when the reader's next sequence has already been overwritten. The stream
// ends when ctx is done or yield returns false.
//
// The subscription point is pinned at call time, so Iterator returns a single-use stream;
// call Iterator again for a fresh subscription.
//
// Each step blocks until its message is written. Breaking out of the range only works
// while the consumer holds a value; parked waiting for the next one it can do nothing, so
// a quiet stream ends through ctx alone. Pass a ctx owned by something outside the loop:
// one created in the calling function can never be cancelled, because the goroutine that
// would cancel it is the one blocked here.
//
// Cancellation cannot interrupt cond.Wait, so an AfterFunc broadcasts the tail slot once
// ctx is done; waitForSequence re-checks ctx under the slot lock before each Wait (see
// wakeWaiters for why that cannot strand a reader).
func (rb *RingBuffer[T]) Iterator(ctx context.Context) iter.Seq[EventOrLag[T]] {
	...
	return func(yield func(EventOrLag[T]) bool) {
		...
		for {
			msg, slotSeq, err := rb.Slot(nextSeq).waitForSequence(ctx, nextSeq)
			if err != nil {
				return
			}
