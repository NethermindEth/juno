...
func BenchmarkHubSend(b *testing.B) {
	for _, v := range defaultVariants() {
		b.Run(v.name, func(b *testing.B) {
			...
			wg.Add(nSubs)
			for _, s := range subs {
				go func(s broadcaster.Subscription[int]) {
					defer wg.Done()
					for range s.Recv() {
					}
				}(s)
		...
		b.Run(fmt.Sprintf("%s/%s", label, v.name), func(b *testing.B) {
			...
			for _, nSubs := range numSubscribers {
				b.Run(fmt.Sprintf("subs=%d", nSubs), func(b *testing.B) {
					...
					for i, s := range subs {
						go func(idx int, s broadcaster.Subscription[T]) {
							defer wg.Done()
							recvd := uint64(0)
							for range s.Recv() {
								recvd++
							}
							perSubRecvd[idx] = recvd
						}(i, s)
					}
					...
					recvRatio := 0.0
					if sent > 0 {
						recvRatio = float64(totalRecvd) / float64(sent*uint64(nSubs))
					}
					...
					b.ReportMetric(recvRatio, "recv_ratio")
