...
func AssertTracedBlockTransactions(
	...
	for description, test := range tests {
		t.Run(description, func(t *testing.T) {
			...
			traces, jErr := handler.TraceBlockTransactions(
				t.Context(),
				rpc.BlockID{Number: test.blockNumber},
			)
...
func TestTraceBlockTransactionsReturnsError(t *testing.T) {
	t.Run("no feeder client set", func(t *testing.T) {
		...
		tracedBlocks, jErr := handler.TraceBlockTransactions(
			t.Context(),
			rpc.BlockID{Number: blockNumber},
		)
...
func TestCall(t *testing.T) {
	...
	t.Run("unknown entrypoint blockifier 0.14.0", func(t *testing.T) {
		...
		mockReader.EXPECT().Network().Return(n)
		mockVM.EXPECT().
			Call(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), false).
