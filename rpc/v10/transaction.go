...
) (sierraClass *core.SierraClass, err error) {
...
func (h *Handler) addToMempool(
	...
	if err = h.memPool.Push(ctx, &mempool.BroadcastedTransaction{
		...
		// set to nil since this field is only present in L1_HANDLER transactions,
		// and, by the v0.10.2 spec, there's no 'starknet_add***transaction' corresponding
		// RPC method for it. L1_HANDLER txs are generated by the sequencer, and are propagated
		// through feeder, P2P, etc..., not by RPC.
		PaidFeeOnL1: nil,
		Proof:       tx.Proof,
	}); err != nil {
		return AddTxResponse{}, rpccore.ErrInternal.CloneWithData(err.Error())
