...
func AdaptFeederBlockTrace(
	...
	for index := range blockTrace.Traces {
		...
		trace := adaptFeederTransactionTrace(transactionTypeFrom(transactions[index]), feederTrace)
...
func adaptCachedBlock(result *tracecache.BlockTrace, returnInitialReads bool) TraceBlockTransactionsResponse {
