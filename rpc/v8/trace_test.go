...
func TestTraceFallback(t *testing.T) {
	...
	for description, test := range tests {
		t.Run(description, func(t *testing.T) {
			...
			// Test that fetching traces from feeder returns an error when no `feederClient` is set
			handler := rpc.New(mockReader, nil, nil, "", nil)
			_, httpHeader, jErr := handler.TraceBlockTransactions(
				context.Background(), rpc.BlockID{Number: test.blockNumber},
			)

			require.Equal(t, rpccore.ErrInternal.Code, jErr.Code)
			assert.Equal(t, httpHeader.Get(rpc.ExecutionStepsHeader), "0")

			// Test traces after having successfully fetched traces from feeder
			handler = handler.WithFeeder(client)
			trace, httpHeader, jErr := handler.TraceBlockTransactions(
				context.Background(), rpc.BlockID{Number: test.blockNumber},
			)
...
func TestTraceTransaction(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		vmTrace.ExecutionResources = &vm.ExecutionResources{
			L1Gas:     utils.Ptr(uint64(2)),
			L1DataGas: utils.Ptr(uint64(3)),
			L2Gas:     utils.Ptr(uint64(4)),
		}
	...
	t.Run("pending block", func(t *testing.T) {
		...
		mockVM.EXPECT().Execute(block.Transactions, []core.Class{declaredClass.Class}, paidL1Fees, &vm.BlockInfo{Header: header},
			...
			Return(vm.ExecutionResults{
				OverallFees:      nil,
				DataAvailability: []core.DataAvailability{{}, {}},
				GasConsumed:      []core.GasConsumed{{L1Gas: 4, L1DataGas: 5, L2Gas: 6}, {}},
		...
		assert.Equal(t, &vm.TransactionTrace{
			...
			ValidateInvocation: &vm.FunctionInvocation{
				ExecutionResources: &vm.ExecutionResources{
					L1Gas: utils.Ptr(uint64(1)),
					L2Gas: utils.Ptr(uint64(2)),
				},
			},
			...
			// Root level execution resources
			ExecutionResources: &vm.ExecutionResources{
				L1Gas:     utils.Ptr(uint64(4)),
				L1DataGas: utils.Ptr(uint64(5)),
				L2Gas:     utils.Ptr(uint64(6)),
			},
