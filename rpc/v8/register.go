...
// RegisterMethods returns the JSON-RPC method bindings for v8. The
...
func (h *Handler) RegisterMethods() []jsonrpc.Method {
	return []jsonrpc.Method{
		...
		jsonrpc.Register("starknet_getBlockWithTxs",
			func(p *BlockIDParams) (*BlockWithTxs, *jsonrpc.Error) {
				return h.BlockWithTxs(p.BlockID)
			}),
		jsonrpc.Register("starknet_getTransactionByHash",
			func(p *TxHashValueParams) (*Transaction, *jsonrpc.Error) {
				return h.TransactionByHash(p.TransactionHash)
			}),
		jsonrpc.Register("starknet_getTransactionReceipt",
			func(p *TxHashValueParams) (*TransactionReceipt, *jsonrpc.Error) {
				return h.TransactionReceiptByHash(p.TransactionHash)
			}),
		jsonrpc.Register("starknet_getBlockTransactionCount",
			func(p *BlockIDValueParams) (uint64, *jsonrpc.Error) {
		...
		jsonrpc.Register("starknet_getTransactionByBlockIdAndIndex",
			func(p *TxByBlockIDAndIndexParams) (*Transaction, *jsonrpc.Error) {
				return h.TransactionByBlockIDAndIndex(p.BlockID, p.Index)
			}),
		jsonrpc.Register("starknet_getStateUpdate",
			func(p *BlockIDValueParams) (*StateUpdate, *jsonrpc.Error) {
				return h.StateUpdate(p.BlockID)
			}),
		...
		jsonrpc.Register("starknet_getStorageAt",
			func(p *StorageAtParams) (*felt.Felt, *jsonrpc.Error) {
				return h.StorageAt(p.ContractAddress, p.Key, p.BlockID)
			}),
		...
		jsonrpc.RegisterC("starknet_getTransactionStatus",
			func(ctx context.Context, p *TxHashValueParams) (*TransactionStatus, *jsonrpc.Error) {
		...
		jsonrpc.RegisterCH("starknet_traceBlockTransactions",
			func(
				...
				return h.TraceBlockTransactions(ctx, p.BlockID)
			}),
		...
		jsonrpc.RegisterC("starknet_subscribeEvents",
			func(ctx context.Context, p *SubscribeEventsParams) (SubscriptionID, *jsonrpc.Error) {
				return h.SubscribeEvents(ctx, p.FromAddress, p.Keys, p.BlockID)
			}),
		...
		jsonrpc.RegisterC("starknet_subscribePendingTransactions",
			func(ctx context.Context, p *SubscribePendingTxsParams) (SubscriptionID, *jsonrpc.Error) {
				return h.SubscribePendingTxs(ctx, p.TransactionDetails, p.SenderAddress)
			}),
		...
		jsonrpc.Register("starknet_getBlockWithReceipts",
			func(p *BlockIDParams) (*BlockWithReceipts, *jsonrpc.Error) {
				return h.BlockWithReceipts(p.BlockID)
			}),
		jsonrpc.Register("starknet_getCompiledCasm",
			func(p *ClassHashParams) (CasmCompiledContractClass, *jsonrpc.Error) {
