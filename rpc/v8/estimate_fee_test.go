...
func TestEstimateFee(t *testing.T) {
	...
	t.Run("ok with zero values", func(t *testing.T) {
		...
		_, httpHeader, err := handler.EstimateFee([]rpc.BroadcastedTransaction{}, []rpcv8.SimulationFlag{}, rpc.BlockID{Latest: true})
		require.Nil(t, err)
	...
	t.Run("ok with zero values, skip validate", func(t *testing.T) {
		...
		_, httpHeader, err := handler.EstimateFee([]rpc.BroadcastedTransaction{}, []rpcv8.SimulationFlag{rpcv8.SkipValidateFlag}, rpc.BlockID{Latest: true})
		require.Nil(t, err)
		assert.Equal(t, httpHeader.Get(rpc.ExecutionStepsHeader), "123")
	})
	...
	t.Run("transaction with invalid contract class", func(t *testing.T) {
		...
		_, _, err := handler.EstimateFee([]rpc.BroadcastedTransaction{invalidTx}, []rpcv8.SimulationFlag{}, rpc.BlockID{Latest: true})
