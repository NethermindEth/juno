...
func TestBlockWithTxsAndReceipts(t *testing.T) {
	...
	for description, id := range errTests {
		t.Run(description, func(t *testing.T) {
			...
			block, rpcErr := handler.BlockWithTxsAndReceipts(id)
			assert.Nil(t, block)
			assert.Equal(t, rpc.ErrBlockNotFound, rpcErr)
		})
	}
	...
	checkLatestBlock := func(t *testing.T, blockWithTxHashes *rpc.BlockWithTxHashes, blockWithTxs *rpc.BlockWithTxs, blockWithTxsAndReceipts *rpc.BlockWithTxsAndReceipts) {
		t.Helper()
		assert.Equal(t, blockWithTxHashes.BlockHeader, blockWithTxs.BlockHeader)
		assert.Equal(t, len(blockWithTxHashes.TxnHashes), len(blockWithTxs.Transactions))
		assert.Equal(t, len(blockWithTxHashes.TxnHashes), len(blockWithTxsAndReceipts.Transactions))
		assert.Equal(t, len(blockWithTxHashes.TxnHashes), len(blockWithTxsAndReceipts.TransactionsWithReceipts))

		for i, txnHash := range blockWithTxHashes.TxnHashes {
			txn, err := handler.TransactionByHash(*txnHash)
			require.Nil(t, err)

			assert.Equal(t, txn, blockWithTxs.Transactions[i], blockWithTxsAndReceipts.Transactions[i], blockWithTxsAndReceipts.TransactionsWithReceipts[i].Transaction)
		}
	}
	...
	fmt.Println(len(latestBlock.Transactions))

	t.Run("blockID - latest", func(t *testing.T) {
		mockReader.EXPECT().Head().Return(latestBlock, nil).Times(3)
		mockReader.EXPECT().L1Head().Return(nil, db.ErrKeyNotFound).Times(3)
		...
		blockWithTxsAndReceipts, rpcErr := handler.BlockWithTxsAndReceipts(rpc.BlockID{Latest: true})
		require.Nil(t, rpcErr)

		checkLatestBlock(t, blockWithTxHashes, blockWithTxs, blockWithTxsAndReceipts)
	})

	t.Run("blockID - hash", func(t *testing.T) {
		mockReader.EXPECT().BlockByHash(latestBlockHash).Return(latestBlock, nil).Times(3)
		mockReader.EXPECT().L1Head().Return(nil, db.ErrKeyNotFound).Times(3)
		...
		blockWithTxsAndReceipts, rpcErr := handler.BlockWithTxsAndReceipts(rpc.BlockID{Hash: latestBlockHash})
		require.Nil(t, rpcErr)
	...
	t.Run("blockID - number", func(t *testing.T) {
		mockReader.EXPECT().BlockByNumber(latestBlockNumber).Return(latestBlock, nil).Times(3)
		mockReader.EXPECT().L1Head().Return(nil, db.ErrKeyNotFound).Times(3)
		...
		blockWithTxsAndReceipts, rpcErr := handler.BlockWithTxsAndReceipts(rpc.BlockID{Number: latestBlockNumber})
		require.Nil(t, rpcErr)
	...
	t.Run("blockID - pending", func(t *testing.T) {
		...
		blockWithTxsAndReceipts, rpcErr := handler.BlockWithTxsAndReceipts(rpc.BlockID{Pending: true})
		require.Nil(t, rpcErr)
