...
func TestAddDeployAccountTransaction(t *testing.T) {
	...
	t.Run("test ErrClassHashNotFound", func(t *testing.T) {
		...
		mockGateway.EXPECT().AddDeployAccountTransaction(deployTxByte).Return(nil, errors.New("Class hash not found"))
	...
	t.Run("ok", func(t *testing.T) {
		...
		mockGateway.EXPECT().AddDeployAccountTransaction(deployTxByte).Return(expectedDeployRespB, nil)
