...
func TestVMErrInternal(t *testing.T) {
	mockCtrl := gomock.NewController(t)
	defer mockCtrl.Finish()
	const network = utils.Mainnet
	mockReader := mocks.NewMockReader(mockCtrl)
	...
	t.Run("trace", func(t *testing.T) {
		mockVM.EXPECT().Execute(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
			Return(nil, nil, vm.ErrInternal)
		mockReader.EXPECT().Receipt(gomock.Any()).Return(nil, nil, uint64(0), nil)
		mockReader.EXPECT().BlockByNumber(gomock.Any()).Return(&core.Block{
			Header: &core.Header{
				Hash:            &felt.Felt{},
				ProtocolVersion: "0.12.3",
			},
		...
		mockReader.EXPECT().StateAtBlockHash(gomock.Any()).Return(mockState, nopCloser, nil)
		mockReader.EXPECT().HeadState().Return(mockState, nopCloser, nil)
		_, rpcErr := handler.TraceTransaction(context.Background(), felt.Felt{})
		require.Equal(t, rpc.ErrInternal.CloneWithData(vm.ErrInternal.Error()), rpcErr)
	})

	t.Run("simulate", func(t *testing.T) {
		mockVM.EXPECT().Execute(nil, nil, uint64(0), uint64(0), gomock.Any(), gomock.Any(), network, []*felt.Felt{}, true, false, true, nil, nil, false).
			Return(nil, nil, vm.ErrInternal)
		mockReader.EXPECT().HeadState().Return(mockState, nopCloser, nil)
		mockReader.EXPECT().HeadsHeader().Return(&core.Header{}, nil)
		_, rpcErr := handler.EstimateFee([]rpc.BroadcastedTransaction{}, []rpc.SimulationFlag{}, rpc.BlockID{Latest: true})
		require.Equal(t, rpc.ErrInternal.CloneWithData(vm.ErrInternal.Error()), rpcErr)
	})
}
