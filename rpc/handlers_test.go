...
func TestSyncing(t *testing.T) {
	...
	synchronizer := sync.New(nil, gw, sync.NopObserver{}, log, time.Duration(0))
...
func TestAddTransaction(t *testing.T) {
	...
	log := utils.NewNopZapLogger()
	handler := rpc.New(nil, nil, utils.MAINNET, mockGateway, nil, nil, nil, "", log)
		...
		t.Run(test.network.String(), func(t *testing.T) {
			...
			t.Run("tx found in db", func(t *testing.T) {
				...
				t.Run("not verified", func(t *testing.T) {
					...
					txStatusCache := cache.NewTransactionStatusCache()
					handler := rpc.New(mockReader, nil, test.network, nil, nil, nil, txStatusCache, "", nil)
			...
			t.Run("transaction not found in db", func(t *testing.T) {
				...
				for description, notFoundTest := range notFoundTests {
					t.Run(description, func(t *testing.T) {
						...
						txStatusCache := cache.NewTransactionStatusCache()
						handler := rpc.New(mockReader, nil, test.network, nil, client, nil, txStatusCache, "", nil)
...
func TestEstimateMessageFee(t *testing.T) {
	...
	handler := rpc.New(mockReader, nil, utils.MAINNET, nil, nil, mockVM, nil, "", log)
	msg := rpc.MsgFromL1{
...
func TestSimulateTransactions(t *testing.T) {
	...
	mockVM := mocks.NewMockVM(mockCtrl)
	log := utils.NewNopZapLogger()
	handler := rpc.New(mockReader, nil, network, nil, nil, mockVM, nil, "", log)
