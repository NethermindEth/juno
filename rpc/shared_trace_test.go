...
func TestSharedTraceCacheInitialReads(t *testing.T) {
	...
	second, steps, err := methods.transaction9(
...
func TestSharedFeederTraceCachePreservesVersionShapes(t *testing.T) {
	...
	a, _, err := methods.block8(t.Context(), &id8)
	require.Nil(t, err)
		...
		t.Run(fmt.Sprintf("feeder=%t", useFeeder), func(t *testing.T) {
			synctest.Test(t, func(t *testing.T) {
				...
				go func() {
					...
					traces, steps, err := methods.block8(t.Context(), &id)
...
func TestSharedTraceCacheReleasesFailedProducer(t *testing.T) {
	...
	methods := registeredTraceMethods(t, h)
	id8 := rpcv8.BlockIDFromHash(header.Hash)
	_, _, err := methods.block8(t.Context(), &id8)
			...
			t.Run(fmt.Sprintf("feeder=%t/producer=%d", useFeeder, producer), func(t *testing.T) {
				...
				for offset := range 3 {
					...
					case 0:
						...
						result, steps, rpcErr = methods.block8(t.Context(), &id)
...
type traceMethods struct {
	...
	transaction8 func(context.Context, felt.Felt) (
		*rpcv8.TransactionTrace,
		http.Header,
		*jsonrpc.Error,
	)
...
func registeredTraceMethods(t *testing.T, h *rpc.Handler) traceMethods {
	...
	bindTraceMethod(t, v8, "starknet_traceBlockTransactions", &methods.block8)
