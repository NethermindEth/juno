...
func TestEstimateFee(t *testing.T) {
	...
	t.Run("ok with zero values", func(t *testing.T) {
		mockVM.EXPECT().
			Execute([]core.Transaction{}, nil, []*felt.Felt{}, &blockInfo, mockState, n, true, false, true, false, true).
	...
	t.Run("ok with zero values, skip validate", func(t *testing.T) {
		mockVM.EXPECT().
			Execute([]core.Transaction{}, nil, []*felt.Felt{}, &blockInfo, mockState, n, true, true, true, false, true).
			Return(vm.ExecutionResults{
				OverallFees:      []*felt.Felt{},
				DataAvailability: []core.DataAvailability{},
				Traces:           []vm.TransactionTrace{},
	...
	t.Run("transaction execution error", func(t *testing.T) {
		...
		require.Equal(
			t,
			rpccore.ErrTransactionExecutionError.CloneWithData(rpcv7.TransactionExecutionErrorData{
				TransactionIndex: 44,
	...
	t.Run("transaction with invalid contract class", func(t *testing.T) {
		...
		_, _, err := handler.EstimateFee(
			[]rpcv7.BroadcastedTransaction{invalidTx},
			[]rpcv6.SimulationFlag{},
			rpcv7.BlockID{Latest: true},
		)
