...
func TestEvents(t *testing.T) {
	...
	args := rpcv7.EventsArg{
		EventFilter: rpcv7.EventFilter{
			FromBlock: &rpcv7.BlockID{Number: 0},
			ToBlock:   &rpcv7.BlockID{Latest: true},
			Address:   from,
			Keys:      [][]felt.Felt{},
		},
		ResultPageRequest: rpcv7.ResultPageRequest{
			ChunkSize:         100,
			ContinuationToken: "",
		},
	}
	...
	t.Run("filter non-existent", func(t *testing.T) {
		t.Run("block number", func(t *testing.T) {
			args.ToBlock = &rpcv7.BlockID{Number: 55}
		...
		t.Run("block hash", func(t *testing.T) {
			args.ToBlock = &rpcv7.BlockID{Hash: new(felt.Felt).SetUint64(55)}
			_, err := handler.Events(args)
			require.Equal(t, rpcv7.ErrBlockNotFound, err)
		})
	})

	t.Run("filter with no from_block", func(t *testing.T) {
		args.FromBlock = nil
		args.ToBlock = &rpcv7.BlockID{Latest: true}
		_, err := handler.Events(args)
		require.Nil(t, err)
	})

	t.Run("filter with no to_block", func(t *testing.T) {
		args.FromBlock = &rpcv7.BlockID{Number: 0}
		args.ToBlock = nil
		_, err := handler.Events(args)
		require.Nil(t, err)
	})
	...
	t.Run("filter with no keys", func(t *testing.T) {
		var allEvents []*rpcv7.EmittedEvent
		...
		t.Run("accumulate events with pagination", func(t *testing.T) {
			var accEvents []*rpcv7.EmittedEvent
			args.ChunkSize = 1
	...
	t.Run("large page size", func(t *testing.T) {
		...
		events, err := handler.Events(args)
		require.Equal(t, rpcv7.ErrPageSizeTooBig, err)
	...
	t.Run("too many keys", func(t *testing.T) {
		...
		events, err := handler.Events(args)
		require.Equal(t, rpcv7.ErrTooManyKeysInFilter, err)
		require.Nil(t, events)
	})
	...
	t.Run("get pending events without pagination", func(t *testing.T) {
		args = rpcv7.EventsArg{
			EventFilter: rpcv7.EventFilter{
				FromBlock: &rpcv7.BlockID{Pending: true},
				ToBlock:   &rpcv7.BlockID{Pending: true},
			},
			ResultPageRequest: rpcv7.ResultPageRequest{
				ChunkSize:         100,
				ContinuationToken: "",
