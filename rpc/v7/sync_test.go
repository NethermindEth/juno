...
func TestSyncing(t *testing.T) {
	...
	handler := rpcv7.New(mockReader, synchronizer, nil, "", utils.Ptr(utils.Mainnet), nil)
	...
	t.Run("undefined starting block", func(t *testing.T) {
		...
		assert.Nil(t, err)
		assert.Equal(t, &rpcv7.Sync{Syncing: &defaultSyncState}, syncing)
	})
