...
func TestVersion(t *testing.T) {
	...
	handler := rpcv7.New(nil, nil, nil, version, nil)
...
func TestSpecVersion(t *testing.T) {
	handler := rpcv7.New(nil, nil, nil, "", nil)
...
func TestThrottledVMError(t *testing.T) {
	...
	handler := rpcv7.New(mockReader, mockSyncReader, throttledVM, "", nil)
	...
	t.Run("trace", func(t *testing.T) {
		...
		_, httpHeader, rpcErr := handler.TraceBlockTransactions(context.Background(), rpcv7.BlockID{Hash: blockHash})
		assert.Equal(t, throttledErr, rpcErr.Data)
		assert.NotEmpty(t, httpHeader.Get(rpcv7.ExecutionStepsHeader))
	})
}
