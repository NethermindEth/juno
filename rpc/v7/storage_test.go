...
func TestStorageProof(t *testing.T) {
	...
	var (
		...
		blockLatest = rpcv7.BlockID{Latest: true}
	...
	t.Run("only unique proof nodes are returned", func(t *testing.T) {
		...
		rootNodes := utils.Filter(proof.ClassesProof, func(h *rpcv7.HashToNode) bool {
	...
	t.Run("contract storage trie address does not exist in a trie", func(t *testing.T) {
		...
		storageKeys := []rpcv7.StorageKeys{{Contract: *contract, Keys: []felt.Felt{*key}}}
	...
	t.Run("contract storage trie key slot does not exist in a trie", func(t *testing.T) {
		...
		storageKeys := []rpcv7.StorageKeys{{Contract: *contract, Keys: []felt.Felt{*noSuchKey}}}
		proof, rpcErr := handler.StorageProof(blockLatest, nil, nil, storageKeys)
		require.NotNil(t, proof)
		require.Nil(t, rpcErr)
		arityTest(t, proof, 0, 0, 0, 1)
...
func TestStorageProof_VerifyPathfinderResponse(t *testing.T) {
	...
	result := rpcv7.StorageProofResult{
		ClassesProof: []*rpcv7.HashToNode{},
		ContractsProof: &rpcv7.ContractProof{
			LeavesData: []*rpcv7.LeafData{
				{
			...
			Nodes: []*rpcv7.HashToNode{
				{
					Node: &rpcv7.BinaryNode{
		...
		ContractsStorageProofs: [][]*rpcv7.HashToNode{},
		GlobalRoots: &rpcv7.GlobalRoots{
...
func TestStorageProof_StorageRoots(t *testing.T) {
	...
	t.Run("get contract proof", func(t *testing.T) {
		...
		expectedResult := rpcv7.StorageProofResult{
			ClassesProof:           []*rpcv7.HashToNode{},
			ContractsStorageProofs: [][]*rpcv7.HashToNode{},
			ContractsProof: &rpcv7.ContractProof{
				LeavesData: []*rpcv7.LeafData{
...
func arityTest(t *testing.T,
	proof *rpcv7.StorageProofResult,
