...
func TestStateUpdate(t *testing.T) {
	...
	for description, id := range errTests {
		t.Run(description, func(t *testing.T) {
			...
			handler := rpcv7.New(chain, mockSyncReader, nil, "", nil)
	...
	handler := rpcv7.New(mockReader, mockSyncReader, nil, "", nil)
	...
	checkUpdate := func(t *testing.T, coreUpdate *core.StateUpdate, rpcUpdate *rpcv7.StateUpdate) {
		t.Helper()
	...
	t.Run("latest", func(t *testing.T) {
		...
		update, rpcErr := handler.StateUpdate(rpcv7.BlockID{Latest: true})
	...
	t.Run("by height", func(t *testing.T) {
		...
		update, rpcErr := handler.StateUpdate(rpcv7.BlockID{Number: uint64(21656)})
		require.Nil(t, rpcErr)
		checkUpdate(t, update21656, update)
	})
	...
	t.Run("by hash", func(t *testing.T) {
		...
		update, rpcErr := handler.StateUpdate(rpcv7.BlockID{Hash: update21656.BlockHash})
		require.Nil(t, rpcErr)
		checkUpdate(t, update21656, update)
	})
	...
	t.Run("post v0.11.0", func(t *testing.T) {
		...
		for name, height := range map[string]uint64{
			...
			t.Run(name, func(t *testing.T) {
				...
				update, rpcErr := handler.StateUpdate(rpcv7.BlockID{Number: height})
				require.Nil(t, rpcErr)
	...
	t.Run("pending", func(t *testing.T) {
		...
		update, rpcErr := handler.StateUpdate(rpcv7.BlockID{Pending: true})
		require.Nil(t, rpcErr)
		checkUpdate(t, update21656, update)
	})
}
