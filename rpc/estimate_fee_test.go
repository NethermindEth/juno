...
func TestEstimateMessageFee(t *testing.T) {
	...
	mockVM.EXPECT().Execute(gomock.Any(), gomock.Any(), gomock.Any(), &vm.BlockInfo{
		Header: latestHeader,
	}, gomock.Any(), &utils.Mainnet, gomock.Any(), false, true, false, false).DoAndReturn(
...
func TestEstimateFee(t *testing.T) {
	...
	t.Run("ok with zero values", func(t *testing.T) {
		mockVM.EXPECT().Execute(nil, nil, []*felt.Felt{}, &blockInfo, mockState, n, true, true, false, false, false).
	...
	t.Run("transaction execution error", func(t *testing.T) {
		...
		mockVM.EXPECT().Execute(nil, nil, []*felt.Felt{}, &blockInfo, mockState, n, false, true, true, false, false).
			Return(nil, nil, vm.TransactionExecutionError{
				Index: 44,
				Cause: errors.New("oops"),
			})
