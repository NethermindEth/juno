...
func TestTransactionReceiptByHash(t *testing.T) {
	...
	t.Run("transaction not found", func(t *testing.T) {
		...
		mockReader.EXPECT().BlockNumberAndIndexByTxHash(txHash).Return(uint64(0), uint64(0), db.ErrKeyNotFound)
		mockSyncReader.EXPECT().PendingData().Return(nil, core.ErrPendingDataNotFound)
		mockReader.EXPECT().HeadsHeader().Return(nil, db.ErrKeyNotFound)
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			mockReader.EXPECT().BlockNumberAndIndexByTxHash(txHash).Return(block0.Number, uint64(test.index), nil)
	...
	t.Run("accepted on l1 receipt", func(t *testing.T) {
		...
		mockReader.EXPECT().BlockNumberAndIndexByTxHash(txHash).Return(block0.Number, uint64(i), nil)
	...
	t.Run("reverted", func(t *testing.T) {
		...
		mockReader.EXPECT().BlockNumberAndIndexByTxHash(revertedTxnHash).Return(
	...
	t.Run("v3 tx", func(t *testing.T) {
		...
		mockReader.EXPECT().BlockNumberAndIndexByTxHash(txnHash).Return(block.Number, uint64(index), nil)
		...
		t.Run(test.network.String(), func(t *testing.T) {
			...
			t.Run("tx found in db", func(t *testing.T) {
				...
				t.Run("not verified", func(t *testing.T) {
					...
					mockReader.EXPECT().BlockNumberAndIndexByTxHash(tx.Hash()).Return(block.Number, uint64(0), nil)
			...
			t.Run("transaction not found in db", func(t *testing.T) {
				...
				for description, notFoundTest := range notFoundTests {
					t.Run(description, func(t *testing.T) {
						mockReader := mocks.NewMockReader(mockCtrl)
						mockSyncReader := mocks.NewMockSyncReader(mockCtrl)
						mockReader.EXPECT().BlockNumberAndIndexByTxHash(notFoundTest.hash).Return(uint64(0), uint64(0), db.ErrKeyNotFound).Times(2)
			...
			t.Run("transaction not found in db and feeder  ", func(t *testing.T) {
				mockReader := mocks.NewMockReader(mockCtrl)
				mockSyncReader := mocks.NewMockSyncReader(mockCtrl)
				mockReader.EXPECT().BlockNumberAndIndexByTxHash(test.notFoundTxHash).Return(uint64(0), uint64(0), db.ErrKeyNotFound)
		...
		t.Run("transaction found in preconfirmed", func(t *testing.T) {
			...
			t.Run("found in candidates", func(t *testing.T) {
				...
				for _, candidateTx := range preConfirmed.CandidateTxs {
					mockReader.EXPECT().BlockNumberAndIndexByTxHash(candidateTx.Hash()).Return(uint64(0), uint64(0), db.ErrKeyNotFound)
...
func TestSubmittedTransactionsCache(t *testing.T) {
	...
	t.Run("transaction not found in db and feeder but found in cache", func(t *testing.T) {
		...
		require.Nil(t, err)
		mockReader.EXPECT().BlockNumberAndIndexByTxHash(res.TransactionHash).Return(uint64(0), uint64(0), db.ErrKeyNotFound)
		mockSyncReader.EXPECT().PendingData().Return(nil, core.ErrPendingDataNotFound).Times(2)
		mockReader.EXPECT().HeadsHeader().Return(nil, db.ErrKeyNotFound).Times(2)
