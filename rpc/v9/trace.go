...
func (h *Handler) TraceTransaction(
	...
	if !isPreConfirmed {
		...
		blockTraces, httphttpHeader, rpcErr := h.traceBlockTransaction(ctx, block)
		if rpcErr != nil {
...
func (h *Handler) TraceBlockTransactions(
	...
	return h.traceBlockTransaction(ctx, block)
}

// traceBlockTransaction gets the trace for a block. The block will always be traced locally except
// on specific case such as with Starknet version 0.13.1 or lower and
func (h *Handler) traceBlockTransaction(
