...
func traceTransactionsWithState(
	...
	declaredClasses, paidFeesOnL1, err := fetchDeclaredClassesAndL1Fees(transactions, classLookupState)
	if err != nil {
...
func (h *Handler) traceInPrelatestBlock(
	...
	traces, httpHeader, rpcErr := traceTransactionsWithState(
		...
		preLatestState, // class lookup state
		blockInfo,
	)
	if rpcErr != nil {
...
func (h *Handler) traceInPreConfirmedBlock(
	...
	state, stateCloser, err := sync.PendingStateBeforeIndex(pendingData, h.bcReader, txIndex)
	if err != nil {
