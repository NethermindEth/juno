...
func TestSimulateTransactions(t *testing.T) {
	...
	defaultMockBehavior := func(
		...
		_ *mocks.MockVM,
		mockState *mocks.MockCommonStateReader,
	) {
	...
	tests := []struct {
		...
		stepsUsed       uint64
		err             *jsonrpc.Error
		mockBehavior    func(*mocks.MockReader, *mocks.MockVM, *mocks.MockCommonStateReader)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			mockState := mocks.NewMockCommonStateReader(mockCtrl)
