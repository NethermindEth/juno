...
// SubscriptionDeduplicationCache is a minimal cache for subscription notifications deduplication.
...
type SubscriptionDeduplicationCache[K comparable, V comparable] struct {
	mu    sync.RWMutex
...
// NewSubscriptionDeduplicationCache creates a new cache.
func NewSubscriptionDeduplicationCache[
	K comparable,
	V comparable,
]() *SubscriptionDeduplicationCache[K, V] {
	cache := &SubscriptionDeduplicationCache[K, V]{}
...
func (c *SubscriptionDeduplicationCache[K, V]) Put(blockNum uint64, key K, value V) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *SubscriptionDeduplicationCache[K, V]) ShouldSend(blockNum uint64, key K, value V) bool {
	c.mu.RLock()
	defer c.mu.RUnlock()
...
func (c *SubscriptionDeduplicationCache[K, V]) Clear() {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *SubscriptionDeduplicationCache[K, V]) findSlotIndex(blockNum uint64) int {
	for i := range c.slots {
...
func (c *SubscriptionDeduplicationCache[K, V]) findSlotToUse() int {
	...
	for i := 1; i < BlockCacheCapacity; i++ {
