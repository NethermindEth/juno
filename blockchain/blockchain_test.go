...
func TestEvents(t *testing.T) {
	...
	t.Run("filter non-existent", func(t *testing.T) {
		...
		t.Run("block number", func(t *testing.T) {
			err = filter.SetRangeEndBlockByNumber(blockchain.To, uint64(44))
			require.Error(t, err)
			err = filter.SetRangeEndBlockByNumber(blockchain.From, uint64(44))
			require.Error(t, err)
		})

		t.Run("block hash", func(t *testing.T) {
			err = filter.SetRangeEndBlockByHash(blockchain.To, &felt.Zero)
			require.Error(t, err)
			err = filter.SetRangeEndBlockByHash(blockchain.From, &felt.Zero)
			require.Error(t, err)
		})
	...
	t.Run("filter with no keys", func(t *testing.T) {
		...
		require.NoError(t, filter.SetRangeEndBlockByNumber(blockchain.From, 0))
		require.NoError(t, filter.SetRangeEndBlockByNumber(blockchain.To, 6))
	...
	t.Run("filter with keys", func(t *testing.T) {
		...
		require.NoError(t, filter.SetRangeEndBlockByNumber(blockchain.From, 0))
		require.NoError(t, filter.SetRangeEndBlockByHash(blockchain.To,
			utils.HexToFelt(t, "0xdc4f7b00430daf9a6985145de533323be2a034252014eb39a958457f690b3b")))
		require.Equal(t, uint64(6), filter.ToBlock)
