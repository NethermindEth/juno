...
func TestCompileSierraToCasm(t *testing.T) {
	t.Run("zero sierra", func(t *testing.T) {
		_, err := vm.CompileSierraToCasm(&starknet.SierraClass{})
		require.Error(t, err)
	})
	...
	t.Run("ok", func(t *testing.T) {
		...
		res, err := vm.CompileSierraToCasm(classDef.Sierra)
		require.NoError(t, err)
	...
	t.Run("declare cairo2 class", func(t *testing.T) {
		...
		raw, err := os.ReadFile(filepath.Join("testdata", "declare_cairo2_definition.json"))
		require.NoError(t, err)
		var definition starknet.SierraClass
		require.NoError(t, json.Unmarshal(raw, &definition))

		_, err = vm.CompileSierraToCasm(&definition)
		require.NoError(t, err)
	})
}
