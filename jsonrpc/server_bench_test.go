...
// Param structs for the bench fixtures.

type benchScalarParams struct {
	N *int `json:"n"`
}

type benchStructParams struct {
	Block *benchBlockID `json:"block"`
}

type benchMixedParams struct {
	N     *int          `json:"n"`
	S     string        `json:"s"`
	Block *benchBlockID `json:"block"`
}

type benchMixedOptParams struct {
	N     *int          `json:"n"`
	S     string        `json:"s,omitempty"`
	Block *benchBlockID `json:"block,omitempty"`
}

type benchValidatedParamWrapper struct {
	P *benchValidatedParam `json:"p"`
}
...
// error paths, batch fan-out, and a response-marshal-heavy axis.
func BenchmarkHandle_HotPath(b *testing.B) {
	b.Run("no_params", func(b *testing.B) {
		s := benchServer(b, false, jsonrpc.Register("noargs",
			func(_ *jsonrpc.NoParams) (int, *jsonrpc.Error) { return 1, nil }))
		runBench(b, s, `{"jsonrpc":"2.0","id":1,"method":"noargs"}`)
	})

	b.Run("one_scalar_pos", func(b *testing.B) {
		s := benchServer(b, false, jsonrpc.Register("scalar",
			func(p *benchScalarParams) (int, *jsonrpc.Error) { return *p.N, nil }))
		runBench(b, s, `{"jsonrpc":"2.0","id":1,"method":"scalar","params":[42]}`)
	})

	b.Run("one_struct_pos", func(b *testing.B) {
		s := benchServer(b, false, jsonrpc.Register("struct1",
			func(p *benchStructParams) (int, *jsonrpc.Error) { return int(p.Block.Number), nil }))
		runBench(b, s, `{"jsonrpc":"2.0","id":1,"method":"struct1","params":[{"block_number":12345}]}`)
	})

	b.Run("three_pos_mixed", func(b *testing.B) {
		s := benchServer(b, false, jsonrpc.Register("mixed",
			func(p *benchMixedParams) (int, *jsonrpc.Error) { return *p.N, nil }))
		runBench(b, s,
			`{"jsonrpc":"2.0","id":1,"method":"mixed","params":[1,"hello",{"block_number":99}]}`)
	})

	b.Run("three_named_required", func(b *testing.B) {
		s := benchServer(b, false, jsonrpc.Register("named",
			func(p *benchMixedParams) (int, *jsonrpc.Error) { return *p.N, nil }))
		runBench(b, s,
	...
	b.Run("named_opt_missing", func(b *testing.B) {
		s := benchServer(b, false, jsonrpc.Register("namedopt",
			func(p *benchMixedOptParams) (int, *jsonrpc.Error) { return *p.N, nil }))
	...
	b.Run("validator_pass", func(b *testing.B) {
		s := benchServer(b, true, jsonrpc.Register("vparam",
			func(p *benchValidatedParamWrapper) (int, *jsonrpc.Error) { return p.P.A, nil }))
	...
	b.Run("response_100_structs", func(b *testing.B) {
		...
		s := benchServer(b, false, jsonrpc.Register("list",
			func(_ *jsonrpc.NoParams) ([]benchSmallStruct, *jsonrpc.Error) { return result, nil }))
