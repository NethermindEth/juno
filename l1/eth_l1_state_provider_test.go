...
func stateUpdateLogJSON(blockNumber, l1RefHeight uint64, removed bool) map[string]any {
	data := make([]byte, 96)
	// globalRoot and blockHash carry distinct per-event values so a
	// transposed field mapping fails the exact-value assertions.
	data[31] = byte(blockNumber & 0xff)
	// blockNumber int256 — low 8 bytes.
	binary.BigEndian.PutUint64(data[56:64], blockNumber)
	data[95] = byte(blockNumber&0xff) + 100
...
func TestEthL1StateProvider_FilterStateUpdate_SendsWellFormedQuery(t *testing.T) {
	contractAddr := eth.AddressFromString("0x000000000000000000000000000000000000beef")
	var captured atomic.Pointer[json.RawMessage]

	srv := clienttest.NewTestServer(t)
	srv.SetHandler(func(req clienttest.TestRequest) (any, *clienttest.TestRPCError) {
		if req.Method == "eth_getLogs" {
			captured.Store(&req.Params[0])
			return []any{}, nil
		}
		return nil, &clienttest.TestRPCError{Code: -32601, Message: req.Method}
	})

	s, err := l1.NewEthL1StateProvider(t.Context(), srv.WSURL(), contractAddr)
	require.NoError(t, err)
	t.Cleanup(s.Close)

	_, err = s.FilterStateUpdate(t.Context(), 100, 200)
	require.NoError(t, err)

	raw := captured.Load()
	require.NotNil(t, raw, "eth_getLogs was never received")
	var q struct {
	...
	assert.Equal(t, "0x64", q.FromBlock)
	assert.Equal(t, "0xc8", q.ToBlock)
	wantAddr, err := contractAddr.MarshalText()
	require.NoError(t, err)
	require.Len(t, q.Address, 1)
	assert.Equal(t, string(wantAddr), q.Address[0])
	require.Len(t, q.Topics, 1)
	assert.Equal(t, contract.LogStateUpdateSigHash.Hex(), q.Topics[0])
}
...
func TestEthL1StateProvider_FilterStateUpdate_DecodesAndTranslates(t *testing.T) {
	...
	assert.Equal(t, *new(felt.Felt).SetUint64(1), got[0].StateRoot)
	assert.Equal(t, *new(felt.Felt).SetUint64(101), got[0].L2BlockHash)
