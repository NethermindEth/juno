...
// finalizedBlockNumber is the geth tag value for the latest finalised
// block; used as the "block number" arg to HeaderByNumber.
...
// generated StarknetFilterer to mirror the pre-removal EthSubscriber
// behaviour. Selected via the --l1-client flag; the hand-rolled
// EthSettlement under l1/eth/client is the other implementation.
//
...
//
// Unlike EthSettlement, this type has no explicit redial state machine:
...
// l1.Client.watchL1StateUpdates already handles. This matches the
// behaviour that shipped on main prior to the hand-rolled migration.
...
// ws/wss is required so log subscriptions work. Signature mirrors
// NewEthSettlement so node.go can branch between backends symmetrically.
func NewGethSettlement(
	...
	if err != nil {
		return nil, fmt.Errorf("parse L1 URL: %w", err)
	}
...
// actually reads. Today only Logs is consumed (per migration.md), but
// every nested Log is converted so the shape matches the hand-rolled
// path exactly.
...
// into l1.StateUpdate as they arrive from the underlying log
// subscription. Mirrors the hand-rolled stateUpdateForwarder.
