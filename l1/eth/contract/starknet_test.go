...
func TestDecode_Success(t *testing.T) {
	// globalRoot = 0x01..01; blockNumber = 0x539 (1337);
	// blockHash = 0x02..02. Both fixtures must stay below the STARK prime.
	data := bytes.Repeat([]byte{0x01}, 32) // globalRoot
	data = append(data, leftPad32Uint64(1337)...)
	data = append(data, bytes.Repeat([]byte{0x02}, 32)...) // blockHash
	...
	wantRoot.SetBytes(bytes.Repeat([]byte{0x01}, 32))
	wantHash.SetBytes(bytes.Repeat([]byte{0x02}, 32))
...
func TestDecode_NonCanonicalFeltRejected(t *testing.T) {
	// A 256-bit value >= the STARK prime must be rejected, not silently
	// reduced into a different felt.
	overP := bytes.Repeat([]byte{0xff}, 32)

	data := make([]byte, 96)
	copy(data[0:32], overP) // globalRoot
	data[63] = 0x01
	_, err := contract.Decode(&eth.Log{
		Topics: []eth.Hash{contract.LogStateUpdateSigHash},
		Data:   eth.DataBytes(data),
	...
	data[31] = 0x01
	data[63] = 0x01
	copy(data[64:96], overP) // blockHash
	_, err = contract.Decode(&eth.Log{
		Topics: []eth.Hash{contract.LogStateUpdateSigHash},
		Data:   eth.DataBytes(data),
	})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "blockHash")
}
