...
func TestFilterLogs_Empty(t *testing.T) {
	...
	logs, err := cli.FilterLogs(t.Context(), client.FilterQuery{FromBlock: 1, ToBlock: 2})
	require.NoError(t, err)
	assert.Empty(t, logs)
}
...
func TestFilterQuery_MarshalShapes(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "minimal",
			q:    client.FilterQuery{FromBlock: 0, ToBlock: 0},
			assert: func(t *testing.T, sent map[string]any) {
...
func TestFilterLogs_ServerError(t *testing.T) {
	...
	_, err := cli.FilterLogs(t.Context(), client.FilterQuery{FromBlock: 1, ToBlock: 1_000_000})
	require.Error(t, err)
